...
def build_llama_mini_program(
    ...
    class LlamaMiniProgram:
        ...
        def kernel_rms_norm(
            ...
            out: pl.Tensor[[seq_len, head_dim], pl.FP32] = pl.store(
                normalized, [0, 0], [seq_len, head_dim], output
            )
            return out
        ...
        def kernel_matmul(
            ...
            out = pl.l0c_store(tile_c_l0c, [0, 0], [seq_len, head_dim], output)
            return out
        ...
        def kernel_lm_head(
            ...
            tile_b_l0b = pl.move(tile_b_l1, target_memory=pl.MemorySpace.Right)
            tile_c_l0c = pl.matmul(tile_a_l0a, tile_b_l0b)
            out = pl.l0c_store(tile_c_l0c, [0, 0], [seq_len, vocab_size], output)
            return out
        ...
        def kernel_matmul_trans_b(
            ...
            out = pl.l0c_store(acc, [0, 0], [seq_len, seq_len], output)
            return out
        ...
        def kernel_softmax(
            ...
            out: pl.Tensor[[seq_len, seq_len], pl.FP32] = pl.store(result, [0, 0], [seq_len, seq_len], output)
            return out
        ...
        def kernel_scale_scores(
            ...
            out: pl.Tensor[[seq_len, seq_len], pl.FP32] = pl.store(scaled, [0, 0], [seq_len, seq_len], output)
            return out
        ...
        def kernel_add(
            ...
            out: pl.Tensor[[seq_len, head_dim], pl.FP32] = pl.store(
                result, [0, 0], [seq_len, head_dim], output
            )
            return out
        ...
        def kernel_rope(
            ...
            out_left: pl.Tensor[[seq_len, head_dim], pl.FP32] = pl.store(
                rotated_left, [0, 0], [seq_len, half_dim], output
            )
            out: pl.Tensor[[seq_len, head_dim], pl.FP32] = pl.store(
                rotated_right, [0, half_dim], [seq_len, half_dim], out_left
            )
            return out
