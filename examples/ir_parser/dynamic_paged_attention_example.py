...
load operations use closure variables captured from build_dynamic_paged_attention_program()
parameters for tile sizes.
...
and capture q_tile, head_dim, block_size as closure variables so that the same
...
def build_dynamic_paged_attention_program(
    ...
    def dyn_kernel_init_inplace(
        ...
        """No-op passthrough for type/shape binding (VECTOR).
    ...
    class DynamicPagedAttentionProgram:
        ...
        and capture q_tile, head_dim, block_size as closure variables so that
        Q_HEADS, HEAD_DIM_DYN, BLOCK_SIZE_DYN (module-level pl.dynamic() vars)
        in their type annotations.  The 4-kernel pipeline and orchestration
        loops are identical to the static version in paged_attention_example.py.
        """
        ...
        def paged_attention(
            ...
            Same 4-stage pipeline as the static version.  InCore kernels are
            called as module-level functions (dyn_kernel_*).
