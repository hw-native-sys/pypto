...
Concepts introduced:
  ...
  - ``mode="mesh"`` (default) / ``mode="ring"``: the same source picks the
    ...
    better one
  - what the builtin narrows: the HOST-builtin ring path is ``Sum`` +
    ``FP32``-only; the InCore composite this example calls (explicit signal)
    accepts the full ``ReduceOp`` family and ``FP16``/``FP32`` in both modes —
    ring keeps the ``[2*(P-1), P]`` signal (the row-per-round signal step 10
    taught), mesh takes ``[P, 1]``
