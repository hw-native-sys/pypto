...
Concepts introduced:
  ...
  - a fresh signal per collective: signal counters are monotonic and do not
    self-reset, so back-to-back collectives each need their own window
...
def build_compose(nr: int):
    """Build the composition program for a compile-time rank count ``nr``.

    A factory rather than a module-level program because every window shape
    depends on the world size (``[nr, 1]`` signals, ``[nr, SIZE]`` gather
    target) and window shapes must be statically known.
    """
