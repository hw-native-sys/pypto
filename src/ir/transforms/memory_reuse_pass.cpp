 ...
 private:
  ...
  void VisitBranchStorage(const StmtPtr& stmt, std::vector<StmtPtr>* allocations) {
    if (auto seq = As<SeqStmts>(stmt)) {
      ...
      if (branch->else_body_) VisitBranchStorage(*branch->else_body_, allocations);
      auto then_yield = FindYieldStmt(branch->then_body_);
      auto else_yield = branch->else_body_ ? FindYieldStmt(*branch->else_body_) : nullptr;
      for (size_t i = 0; i < branch->return_vars_.size(); ++i) {
        ...
        CHECK_SPAN(then_yield && else_yield && i < then_yield->value_.size() && i < else_yield->value_.size(),
                   branch->span_)
        ...
        RetargetBranchProducer(else_var, *branch->else_body_, target, memory);
      }
    }
  }
