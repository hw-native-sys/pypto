...
void RejectFp4UnsupportedLayout(const std::optional<TensorView>& tensor_view,
  ...
  if (tensor_view.has_value()) {
    ...
    CHECK_SPAN(!IsMxTensorLayout(tensor_view->layout), span)
        << "PackFp4: FP4 MX layouts (MX_A_ZZ / MX_B_NN) are unsupported; last-axis packing does not "
           "match MX block geometry (see docs/en/dev/fp4.md)";
  }
...
std::optional<TileView> PackTileView(std::optional<TileView> view, const Span& span) {
  ...
  // Linear start_offset has no axis metadata; non-zero values cannot be safely halved.
  if (view->start_offset) {
    auto offset = As<ConstInt>(view->start_offset);
    CHECK_SPAN(offset && offset->value_ == 0, span)
        << "PackFp4: TileView start_offset must be null or ConstInt(0) for FP4 packing; "
           "non-zero offsets are not rewritten (see docs/en/dev/fp4.md)";
  }
  view->valid_shape = HalveLastAxisVector(std::move(view->valid_shape), LastAxisKind::kCoveringSize, span);
  view->stride = HalveLeadingStrides(view->stride, span);
  return view;
...
void RejectUnrewrittenFp4CoordArgs(const OpPtr& op, const Span& span, const std::vector<ExprPtr>& args) {
...
bool CallTouchesFp4Family(const TypePtr& result_type, const std::vector<ExprPtr>& args,
                          const std::vector<std::pair<std::string, std::any>>& kwargs) {
  if (TypeHasFp4Family(result_type)) return true;
  for (const auto& arg : args) {
...
bool CallNeedsLastAxisPack(const TypePtr& result_type, const std::vector<ExprPtr>& args,
                           const std::vector<std::pair<std::string, std::any>>& kwargs) {
  if (TypeHasLogicalFp4(result_type)) return true;
  for (const auto& arg : args) {
...
/// Ops with no last-axis shape/offset operands — skip RejectUnrewrittenFp4CoordArgs.
bool OpSkipsFp4CoordRewrite(const OpPtr& op) {
  if (As<GlobalVar>(op)) return true;
  // Elementwise / create-like ops that may carry INT scalars or shape tuples that
  // are not last-axis packing coordinates for the FP4 carrier axis.
  return IsOp(op, "tile.cast") || IsOp(op, "tensor.cast") || IsOp(op, "tile.add") || IsOp(op, "tile.sub") ||
         IsOp(op, "tile.mul") || IsOp(op, "tile.div") || IsOp(op, "tile.neg") || IsOp(op, "tile.exp") ||
         IsOp(op, "tile.abs") || IsOp(op, "tile.relu") || IsOp(op, "tile.recip") || IsOp(op, "tile.rsqrt") ||
         IsOp(op, "tile.sqrt") || IsOp(op, "tile.log") || IsOp(op, "tile.sin") || IsOp(op, "tile.cos") ||
         IsOp(op, "tile.move");
}

void RejectUnsupportedFp4FamilyOps(const OpPtr& op, const Span& span, const TypePtr& result_type,
                                   const std::vector<ExprPtr>& args) {
  if (!TypeHasFp4Family(result_type) && !std::any_of(args.begin(), args.end(), [](const ExprPtr& a) {
        return a && TypeHasFp4Family(a->GetType());
      })) {
    return;
  }
  auto reject = [&](const char* reason) {
    CHECK_SPAN(false, span) << "PackFp4: " << op->name_ << " is not supported for FP4/FP4E2M1X2 (" << reason
                            << "; see docs/en/dev/fp4.md)";
  };
  if (IsOp(op, "tensor.transpose") || IsOp(op, "tile.transpose")) {
    reject("transpose is not supported for FP4 in this release");
  }
  if (IsOp(op, "tensor.reshape") || IsOp(op, "tile.reshape")) {
    reject("reshape is not supported for FP4 in this release");
  }
  if (IsOp(op, "tensor.read") || IsOp(op, "tensor.write") || IsOp(op, "tile.read") ||
      IsOp(op, "tile.write")) {
    reject("scalar read/write would change nibble vs packed-pair semantics");
  }
  // Shape query: last-axis dim is nibble vs carrier ambiguous after packing; keep this
  // release conservative — use static shape literals instead of tensor.dim on FP4.
  if (IsOp(op, "tensor.dim")) {
    reject("tensor.dim on FP4/FP4E2M1X2 is not supported in this release (use static shapes)");
  }
  if (IsOp(op, "pld.tile.remote_load") || IsOp(op, "pld.tile.remote_store") ||
      IsOp(op, "pld.tensor.remote_store") || IsOp(op, "pld.tile.put") || IsOp(op, "pld.tile.get") ||
      IsOp(op, "pld.tensor.put") || IsOp(op, "pld.tensor.get") || IsOp(op, "pld.tensor.window")) {
    reject("distributed / multi-device FP4 is TODO");
  }
  // GM partition paths without ExpandPackedFp4GmLastAxis — hard-reject until Expand lands.
  if (IsOp(op, "tile.mgather") || IsOp(op, "prefetch.async_prefetch")) {
    reject("mgather/prefetch FP4 is not supported in this release (no packed GM expand)");
  }
  if (IsOp(op, "tile.ci") || IsOp(op, "tile.tri") || IsOp(op, "tile.random")) {
    reject("tile.ci/tri/random FP4 is not supported in this release");
  }
}

std::vector<ExprPtr> RewriteCallArgs(const OpPtr& op, const Span& span,
                                     const CallPtr& original_for_reinterpret,
                                     const std::vector<ExprPtr>& in_args, bool rewrite_coords) {
  std::vector<ExprPtr> args = in_args;
  // Cross-function calls and ops without packed-axis coordinates: do not treat
  // INT/INDEX scalars as unrewritten last-axis coords.
  if (OpSkipsFp4CoordRewrite(op)) return args;
  ...
  if (IsOp(op, "tensor.slice") || IsOp(op, "tile.slice")) {
    ...
    rewrite_tuple(1, LastAxisKind::kOffset);
    rewrite_tuple(3, LastAxisKind::kCoveringSize);
  } else if (IsOp(op, "tensor.assemble") || IsOp(op, "tile.assemble")) {
    rewrite_tuple(2, LastAxisKind::kOffset);
  } else if (IsOp(op, "tensor.view")) {
    rewrite_tuple(1, LastAxisKind::kCoveringSize);
    rewrite_tuple(2, LastAxisKind::kCoveringSize);  // optional valid_shape
  } else if (IsOp(op, "tensor.fillpad_expand") || IsOp(op, "tile.fillpad_expand")) {
    rewrite_tuple(1, LastAxisKind::kCoveringSize);
  } else if (IsOp(op, "tile.reinterpret_view") || IsOp(op, "tensor.reinterpret_view")) {
    if (rewrite_coords && original_for_reinterpret &&
        ReinterpretViewNeedsShapePack(original_for_reinterpret)) {
      rewrite_tuple(1, LastAxisKind::kCoveringSize);
    }
  } else if (IsOp(op, "tensor.create") || IsOp(op, "tile.create") || IsOp(op, "tensor.full") ||
             IsOp(op, "tile.full")) {
  ...
  } else if (IsOp(op, "tensor.set_validshape") || IsOp(op, "tile.set_validshape")) {
    rewrite_scalar(2, LastAxisKind::kCoveringSize);
  } else if (IsOp(op, "tile.extract")) {
    rewrite_scalar(2, LastAxisKind::kOffset);
    rewrite_tuple(3, LastAxisKind::kCoveringSize);
  } else if (IsOp(op, "system.cacheinvalid")) {
    if (args.size() >= 3) {
      rewrite_tuple(1, LastAxisKind::kCoveringSize);
      rewrite_tuple(2, LastAxisKind::kOffset);
    }
  } else {
    RejectUnrewrittenFp4CoordArgs(op, span, args);
  }
  return args;
}

TypePtr RebuildTileCastContiguousStride(const TypePtr& type, const Span& span) {
  auto tile = As<TileType>(type);
  if (!tile || !tile->dtype_.IsPackedFp4() || !tile->tile_view_.has_value()) return type;
  TileView view = *tile->tile_view_;
  view.stride = RowMajorStridesFromShape(tile->shape_, span);
  return std::make_shared<TileType>(tile->shape_, tile->dtype_, tile->memref_, view, tile->memory_space_);
}
...
class PackFp4Mutator : public IRMutator {
 public:
  ...
  ExprPtr VisitExpr_(const CallPtr& op) override {
    ScopedTypeSpan scope(this, op->span_);
    RejectUnsupportedFp4FamilyOps(op->op_, op->span_, op->GetType(), op->args_);
    const bool pack_coords = CallNeedsLastAxisPack(op->GetType(), op->args_, op->kwargs_);
    const bool touches_fp4_family = CallTouchesFp4Family(op->GetType(), op->args_, op->kwargs_);
    ...
    TypePtr new_type = call->GetType();
    bool changed = kwargs_changed;
    if (pack_coords) {
      new_args = RewriteCallArgs(call->op_, call->span_, op, call->args_, /*rewrite_coords=*/true);
      changed = true;
      ...
      RewriteCallArgs(call->op_, call->span_, op, call->args_, /*rewrite_coords=*/false);
    }
    // Fresh tile.cast results must be contiguous after packing (do not inherit
    // scaled non-contiguous source strides). Hand-built IR may still carry
    // pre-Pack non-contiguous strides; DeduceTileCastType already uses row-major
    // for logical FP4 targets.
    if (IsOp(call->op_, "tile.cast")) {
      auto rebuilt = RebuildTileCastContiguousStride(new_type, call->span_);
      if (rebuilt.get() != new_type.get()) {
        new_type = std::move(rebuilt);
        changed = true;
      }
    }
  ...
  ExprPtr VisitExpr_(const SubmitPtr& op) override {
    ScopedTypeSpan scope(this, op->span_);
    // Submit is always a GlobalVar (user function) launch — same FP4 policy as Call.
    RejectUnsupportedFp4FamilyOps(op->op_, op->span_, op->GetType(), op->args_);
    const bool pack_coords = CallNeedsLastAxisPack(op->GetType(), op->args_, op->kwargs_);
    const bool touches_fp4_family = CallTouchesFp4Family(op->GetType(), op->args_, op->kwargs_);
    ExprPtr visited = IRMutator::VisitExpr_(op);
    auto submit = As<Submit>(visited);
    INTERNAL_CHECK_SPAN(submit, op->span_) << "PackFp4: Submit mutation must remain a Submit";
    auto [new_kwargs, kwargs_changed] = RewriteDtypeKwargs(submit->kwargs_);
    std::vector<ExprPtr> new_args = submit->args_;
    bool changed = kwargs_changed;
    if (pack_coords) {
      new_args = RewriteCallArgs(submit->op_, submit->span_, /*original_for_reinterpret=*/nullptr,
                                 submit->args_, /*rewrite_coords=*/true);
      changed = true;
    } else if (touches_fp4_family) {
      RewriteCallArgs(submit->op_, submit->span_, nullptr, submit->args_, /*rewrite_coords=*/false);
    }
    if (!changed) return visited;
    return std::make_shared<Submit>(submit->op_, std::move(new_args), submit->deps_, std::move(new_kwargs),
