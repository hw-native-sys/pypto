...
ExprPtr ResultAliasedDestination(const CallPtr& call) {
  ...
  auto destination = [&](size_t index) -> ExprPtr {
    if (index >= call->args_.size()) return nullptr;
    // An aliased destination is by definition an argument the operator writes.
    // Reading the two declarations together keeps a future edit to either one
    // from silently disagreeing with the other.
    INTERNAL_CHECK_SPAN(ArgEffectWrites(entry->GetArgEffect(index, call->kwargs_)), call->span_)
        << "Internal error: operator '" << call->op_->name_ << "' aliases its result to argument " << index
        << " but does not declare a write effect on it";
    return call->args_[index];
  };
  ...
  // Cross-rank pushes and pulls return the window they landed bytes in:
  //   pld.tile.remote_store(src, target, peer, offsets)  -> target
  //   pld.tile.put/get(dst, peer, src, stage, ...)       -> dst
