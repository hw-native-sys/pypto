...
std::vector<IterArgPtr> RebuildLane1LoopIterArgs(const std::vector<IterArgPtr>& iter_args,
  ...
  for (const auto& iter_arg : iter_args) {
    ...
    if (new_init != iter_arg->initValue_ || new_type != iter_arg->GetType()) {
      auto new_iter_arg = std::make_shared<IterArg>(iter_arg->name_hint_, new_type, new_init, iter_arg->span_);
...
std::vector<StmtPtr> BuildNoSplitLane1ReplayStmts(const std::vector<StmtPtr>& stmts,
  ...
  for (const auto& stmt : stmts) {
    ...
    if (auto for_stmt = std::dynamic_pointer_cast<const ForStmt>(stmt)) {
      ...
      auto new_iter_args =
          RebuildLane1LoopIterArgs(for_stmt->iter_args_, replacements, loop_replacements);
    ...
    if (auto while_stmt = std::dynamic_pointer_cast<const WhileStmt>(stmt)) {
      auto loop_replacements = replacements;
      auto new_iter_args =
          RebuildLane1LoopIterArgs(while_stmt->iter_args_, replacements, loop_replacements);
      auto new_return_vars = RebuildLane1LoopReturnVars(while_stmt->return_vars_, new_iter_args, replacements);
      auto new_body_stmts =
