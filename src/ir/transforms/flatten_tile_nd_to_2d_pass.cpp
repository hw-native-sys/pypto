...
class PreconditionChecker : public IRVisitor {
 ...
 private:
  ...
  void CheckCall(const CallPtr& call) {
    ...
    // Disallow tile.slice on >2D tiles (tile.read/tile.write are handled by the pass)
    if (name == "tile.slice") {
      if (!call->args_.empty()) {
...
std::vector<StmtPtr> TransformBody(const std::vector<StmtPtr>& stmts, FlattenContext& ctx,
  ...
  for (const auto& stmt : stmts) {
    ...
    if (auto eval = As<EvalStmt>(stmt)) {
      // tile.write in EvalStmt on >2D tiles: flatten ND indices to 2D
      if (auto call = As<Call>(eval->expr_)) {
        if (call->op_ && call->op_->name_ == "tile.write") {
          ...
          if (orig_tile_type && IsNdTile(orig_tile_type)) {
            ...
            auto new_call = op_registry.Create("tile.write", new_args, call->kwargs_, span);
    ...
    if (op_name == "tile.store") {
      ...
      if (orig_tile_type && out_tensor_type && out_tensor_type->shape_.size() > 2) {
        ...
        // Ensure offsets rank matches shapes rank (DeduceTileStoreType requirement).
        auto offsets_tuple = As<MakeTuple>(new_args[1]);
        if (offsets_tuple && offsets_tuple->elements_.size() < tensor_rank) {
          std::vector<ExprPtr> padded_offsets;
          padded_offsets.reserve(tensor_rank);
          size_t pad = tensor_rank - offsets_tuple->elements_.size();
          for (size_t i = 0; i < pad; ++i) {
            padded_offsets.push_back(std::make_shared<ConstInt>(0, DataType::INDEX, span));
          }
          for (const auto& off : offsets_tuple->elements_) {
            padded_offsets.push_back(off);
          }
          new_args[1] = std::make_shared<MakeTuple>(padded_offsets, span);
        }
      }
