...
std::vector<std::pair<const MemRef*, MemRefPtr>> AddAllocPass::AllocateMemoryAddresses(
...
std::vector<StmtPtr> AddAllocPass::CreateAllocStatements(
...
StmtPtr AddAllocPass::PrependAllocStatements(const StmtPtr& body, const std::vector<StmtPtr>& alloc_stmts) {
...
FunctionPtr AddAllocPass::Run(const FunctionPtr& func) {
  // If addOp_ is false, return the function without modification
  if (!addOp_) {
    return func;
  }
