...
// promise. Pinning the op to the AIV lane (which the pass 24 placement stamp
...
class SplitAivStructuralVerifier : public IRVisitor {
 public:
  ...
  // once it stopped.
  //
  // Reporting here rather than at LowerAutoVectorSplit (pass 24) puts the
  ...
  void VisitStmt_(const SplitAivScopeStmtPtr& op) override {
    ...
    // the same reason check (h) lives here: LowerAutoVectorSplit (pass 24) keeps
