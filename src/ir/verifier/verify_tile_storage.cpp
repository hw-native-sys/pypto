 ...
 private:
  ...
  void CheckDestinations(const std::vector<TileTypePtr>& tiles, const Span& span, bool allow_identical) {
    ...
    for (const auto& tile : tiles) {
      ...
      const auto memory_space = tile->GetMemorySpace();
      if (!memory_space) {
        Error("On-chip tile storage must have a MemRef and memory space", span);
        return;
      }
      groups[{*memory_space, physical_ ? nullptr : memref->base_.get()}].push_back(tile);
    }
