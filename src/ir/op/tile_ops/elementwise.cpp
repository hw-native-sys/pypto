...
TypePtr DeduceTileSelectType(const std::vector<ExprPtr>& args,
  ...
  // Rank 2 only. `tile.sels` accepts nothing else, and `FlattenTileNdTo2D` --
  // which would otherwise collapse an ND operand -- runs AFTER LowerCompositeOps
  // and does not cover the cmp/sel family anyway, so an ND select fails either
  // in this op's own lowering or later at the TileOps2D verifier. Reject it here
  // so the diagnostic names the call instead of a pass the author never invoked.
  constexpr size_t kSelectRank = 2;
  for (size_t i = 0; i < args.size(); ++i) {
    auto tile_arg = As<TileType>(args[i]->GetType());
    if (!tile_arg) continue;
    CHECK_SPAN(tile_arg->shape_.size() == kSelectRank, args[i]->span_)
        << "The operator " << op_name << " requires rank-2 operands, but argument " << i << " has rank "
        << tile_arg->shape_.size() << " " << FormatShape(tile_arg->shape_)
        << "; reshape to 2D before selecting";
  }
