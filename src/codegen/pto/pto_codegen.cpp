...
static bool ShouldAliasScatterUpdateResultToInput(const AssignStmtPtr& stmt) {
...
void PTOCodegen::VisitStmt_(const AssignStmtPtr& op) {
  bool alias_scatter_result_to_input = ShouldAliasScatterUpdateResultToInput(op);
  if (auto tile_type = ir::GetTileTypeWithMemRef(op->var_->GetType())) {
    if (fs_.tpop_result_vars.count(op->var_.get()) == 0 && !alias_scatter_result_to_input) {
