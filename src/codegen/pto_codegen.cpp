...
std::map<std::string, std::string> PTOCodegen::Generate(const ProgramPtr& program) {
  // Clear state
  code_lines_.clear();
  tile_decls_.clear();
  ...
  // Generate kernel functions (PTO assembly) to separate .pto files in kernels/ subdirectory
  for (const auto& func : kernel_functions) {
    ...
    // Save to .pto file in kernels/ subdirectory
    std::string filename = "kernels/" + func->name_ + ".pto";
    files[filename] = oss.str();
  }

  // Generate orchestration function (C++ Runtime code) using shared orchestration codegen
  if (!orchestration_functions.empty()) {
    if (orchestration_functions.size() > 1) {
      throw ValueError("Program should have exactly one orchestration function, got " +
    ...
    std::string orch_code = codegen::GenerateOrchestration(program, func);
    oss << orch_code;

    // Save to .cpp file in orchestration/ subdirectory
...
void PTOCodegen::PrepareForFunction(const FunctionPtr& func) {
  code_lines_.clear();
  tile_decls_.clear();
  scalar_decls_.clear();
  var_to_physical_tile_.clear();
