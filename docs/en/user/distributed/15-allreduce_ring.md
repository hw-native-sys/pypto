...
per-step transfer stays `N/P` no matter how large `P` gets. Synchronization is
...
`N/P` — the per-step size is constant as `P` grows, which is what keeps the
ring efficient at large world sizes.
...
- **Neighbour-ready handshakes, not barriers.** Each round gets its own signal
  ...
  two adjacent ranks ever synchronize — O(P) signals total, not the O(P²) a
  full barrier per round would cost.

**Cost card (per rank):** `2 * (P-1) / P * N` total — the same as two-phase —
but in `2*(P-1)` steps of `N/P` bytes each. The per-step size is **constant
as P grows**, unlike mesh's `(P-1) * N` per step; that is the ring's reason to
exist.

## Edge cases

> **Fatal pitfall — a negative left-neighbour index.** `(my_rank - 1) %
> nranks` is `-1` at rank 0 under truncating modulo: the `remote_load` then
> targets an invalid peer and the ring deadlocks at P=2 (the only case where
> rank 0's left neighbour is itself). **Fix:** always write
> `(my_rank - 1 + nranks) % nranks`.
