...
Codegen: an orchestration `tensor.slice` with `drop_dims` emits a full-rank `Tensor::view(shapes, offsets)` (the runtime view loops over the parent rank) followed by a rank-reduced `Tensor::reshape(reduced_shapes, reduced_ndim)` that reinterprets the unit-extent view at the result rank (when every axis is dropped — `C[i]` on a 1D tensor — the 0D result is emitted as `reshape(nullptr, 0)`, since a zero-length C++ shape array is ill-formed). `reshape` is zero-copy only on a contiguous view, so this lowering requires both: (1) the dropped axes form a **leading prefix** `{0, 1, ..., k-1}` (numpy subscript sugar like `C[i]` / `C[i, j, :, :]`), and (2) every **kept axis after the outermost** spans its full extent — in a row-major view only the outermost kept axis may be partially sliced. Violations are rejected with actionable errors instead of emitting code that asserts at runtime: dropping a **non-leading** axis (e.g. `C[:, i, :]` → `drop_dims=[1]`) gives a strided view, and a partially-sliced **inner** kept axis (e.g. `C[i, :, 2:6]`) is also strided. Both need the per-axis index lowering the distributed `tensor.slice` handler uses — a follow-up for orchestration. A rank-reducing `tile.slice` on a **>2D** tile (e.g. subscripting a literal `>2D pl.Tile`) is not yet lowered — `FlattenTileNdTo2D` rejects it with an actionable error — because it would require merging the sliced source window to 2D before re-deriving `drop_dims`; slice into a 2D tile instead.
