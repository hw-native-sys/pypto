...
| `pld.tensor.all_to_all` | push-based symmetric personalized exchange — every rank pushes its per-destination chunks to every peer's window, returns window as result | `DistributedTensorType` (same as src) | composite |
