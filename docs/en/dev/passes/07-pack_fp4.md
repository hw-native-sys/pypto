...
Carrier versus nibble units: [FP4 unit convention](../fp4.md#unit-convention).
...
5. **Call operands**: rewrite shape / offset / size tuples on the whitelist
   ...
   create, full, …). **Reject** reshape / transpose / read / write / `tensor.dim` /
   remote / window / put / get. Unlisted ops that pass a non-empty coordinate
   tuple **or** an integer / `INDEX` scalar coordinate fail loudly.
