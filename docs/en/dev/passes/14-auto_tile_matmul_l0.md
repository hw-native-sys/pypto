...
L0 tiling for `tile.matmul` / `tile.matmul_acc` ops with a Mat right operand (and a Mat- or Vec-resident left operand): pick an L0 tile shape `(m, n, k)` from the active backend's L0 capacities and rewrite the call into pipelined L0 sub-tiles with Mat→Left/Right `tile.extract`s. M/N tiling is used both when the output exceeds L0c and for the Vec-left PV specialization whose stage-2 Right panel would exceed L0B.
...
**K-tiling vs M/N-tiling.** When the chooser returns `m == M` and `n == N`, only K is tiled. When it returns `m < M` or `n < N`, the pass computes a `ceil(M/m) × ceil(N/n)` grid of `[m, n]` sub-tiles (partial on the boundary — `m`/`n` need not divide `M`/`N`). This commonly keeps an oversized output Acc within L0c. It also covers Vec-left PV: if the staged full A panel fits but a stage-2 `[K,N]` Right panel exceeds L0B, the pass asks `ChooseL0Tile` for the N-only, full-K, output-stationary design (`m=M`, `k=K`), preserving the planner-selected dbC gate. For `[32,128] @ [128,512]` BF16 on A2/A3 this yields `[128,128]` Right tiles: 32 KiB per stage, exactly 64 KiB for two stages. The output tensor is chained through the generated stores in SSA form.
...
2. **Pick L0 tile shape** — call `utils::ChooseL0Tile(cfg)` with the active `BackendHandler`'s `GetL0{a,b,c}CapacityBytes()`, `GetL0FractalAlignment()` / `GetMinL0TileDim()`, and `GetL0CostModel()` (L1↔L0 bandwidths + MAD issue overhead), plus per-operand element width (`bytes_a/b/c`) read from the call's result type so the chooser sees the actual accumulator footprint. `c_read = is_matmul_acc` because `tile.matmul_acc` threads the caller's accumulator through the K-loop's iter-arg (γ_C = 2, doubling the C traffic the model charges). The chooser returns `(m, n, k)` plus the chosen design point — an **exhaustive roofline-`wall` minimum**, not a closed form. For the Vec-left PV stage-2 overflow described above, a second constrained chooser call fixes the realizable axes `m=M`, `k=K`, disables operand-stationary (Right must remain double-buffered), and retains the PassContext-selected `allow_double_buffer_c`; see [Cost model & design space](#cost-model--design-space-choosel0tile) below.
...
5. **Build the K-loop** (per output sub-tile — the whole output when K-only, or each `[m, n]` sub-tile when M/N tiling):
   ...
   - **Vec left operand staging** — when the left (A) operand is `Vec`-resident (PV / `score·V`), a single `tile.move(lhs, target_memory=Mat)` is emitted **before the entire K or M/N grid** and every Left extract slices from that staged Mat tile. Keeping the Vec→Mat crossing a `tile.move` lets [`ExpandMixedKernel`](19-expand_mixed_kernel.md) recognise it (`CollectCVBoundaryMoves` only matches `tile.move`) and lower it to the cross-core `tpop_from_aiv` handshake (which lands the data in Mat). Extracting straight from the Vec tile would instead leave the operand a dangling cross-boundary free variable on the cube side.
...
For Vec-left PV `[32,128] @ [128,512]` BF16, the left tile is moved to Mat once and the N loop extracts `[128,128]` Right panels. If the original result feeds `slice[0:16,:]` and `slice[16:32,:]`, each `[32,128]` Acc tile is sliced into two `[16,128]` windows and stored at the current N offset; no full `[32,512]` Vec scratch is created.
...
| Vec-left PV `tile.matmul` whose stage-2 full-N Right panel exceeds L0B, consumed by one full store or bounded disjoint constant full-N row-slice stores | The Vec left tile is staged once; `ChooseL0Tile` selects an N-only/full-K output-stationary grid, and every requested store is distributed across the N tiles |
...
| M/N tiling is selected but no placement applies — `matmul_acc`, unsupported/overlapping/dynamic PV slices, another non-matmul-operand consumer, or a chained-matmul scratch whose `[M, N]` exceeds Mat/L1 | Skipped with `PerfHint` (`PH-AT-006`) |
...
| `PH-AT-006` | The chooser selected M/N tiling but no placement applies — `tile.matmul_acc`; overlapping, dynamic, partial-N, unchained, or otherwise unsupported PV slice consumers; or another result consumed on-chip that is **not** *entirely* a matmul operand. A result consumed entirely as a matmul operand takes the **Mat-scratch** path (no hint), unless its `[M, N]` scratch exceeds Mat/L1 capacity. |
