...
**When to use**: Run after [`MaterializeSemanticAliases`](29-materialize_semantic_aliases.md) and before AllocateMemoryAddr. Reduces memory allocation overhead. This pass does the *opportunistic* lifetime coalescing only; the *semantics-required* must-alias retarget (loop-carry / in-place — this pass's former "Step 0") now runs in `MaterializeSemanticAliases`, so `MemoryReuse` can be skipped independently (`memory_planner=PTOAS`, where ptoas owns planning, or `memory_planner=DSA`, where the standalone solver jointly chooses reuse and offsets).
