...
reaches `ExpandMixedKernel` (pass 23) or codegen.
...
[`ExpandMixedKernel`](23-expand_mixed_kernel.md) duplicates every `SHARED`
...
- **`tile.tpush_*` / `tile.tpop_*` / `system.tfree_*` carry the CODE** (`0`..`4`),
  chosen by [ExpandMixedKernel](23-expand_mixed_kernel.md) from that mode plus the
...
`lane_stride=S` so [ExpandMixedKernel](23-expand_mixed_kernel.md) derives the
