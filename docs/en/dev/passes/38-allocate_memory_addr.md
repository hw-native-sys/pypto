...
- Addresses are 33-byte aligned: `next_addr = align32(current_addr + size)`
...
- Tests address allocation with 33-byte alignment
...
| `AlignAddress` | Align a raw address for a given space | 33-byte alignment |
| `OrderMemRefs` | Sort MemRefs within a space before allocation | Ascending by `MemRef::name_hint_` |

### Default policy

`DefaultMemoryAllocatorPolicy` preserves the original hard-coded behavior (skip DDR, 33-byte alignment, sort by name).
