...
The pass also handles an already-tiled form: a user-authored static `pl.pipeline(stage=2)` with one `tile.matmul(Left, Right)` per iteration and a direct `tile.store` or `tile.assemble` drain. If two static Acc result tiles fit in L0C, AutoTile marks that loop for L0C double buffering. The existing pipeline lowering then emits two co-live accumulators and schedules `matmul, matmul, drain, drain`, overlapping tile *i*'s FIXPIPE drain with tile *i+1*'s MAD. A loop-invariant operand remains shared; only operands moved by the pipeline and the two Acc results are ping-ponged. Multiple matmuls, nested control flow, indirect result uses, and an Acc tile larger than L0C/2 remain unchanged.
...
**When to use**: Always, as part of the default tile-stage pipeline. The pass is a no-op when no Mat-resident matmul needs tiling or cast folding and no existing stage-2 L0 pipeline qualifies for automatic accumulator double buffering.
...
8. **Recognize existing stage-2 L0 pipelines** — independently of chooser-driven rewrites, inspect each static `ForKind::Pipeline` with `pipeline_stages=2` and at least two iterations. If its direct body has no nested control flow, contains exactly one plain `tile.matmul` with static `Left`/`Right` operands, produces a static 2D Acc tile no larger than `L0C/2`, and uses that result exactly once in a direct `tile.store` or `tile.assemble`, attach `pipeline_double_buffer_c=true` and `pipeline_overlap_stores=false`. Explicitly attributed loops are left unchanged. This is a local capacity-and-schedule decision; a body with multiple independent matmuls is deferred because it needs a joint buffer plan.
