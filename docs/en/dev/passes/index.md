...
| 51 | [InsertCommFence](51-insert_comm_fence.md) | Phase A: marks each publishing write and each wait inside InCore bodies; phase B: prepends invalidate-only whole-GM `system.cacheinvalid` at InCore entry when the body scalar-reads (`tensor.read`) a `DistributedTensorType` value; notify itself gets no marker |
