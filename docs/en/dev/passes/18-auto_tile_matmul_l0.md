...
L0 tiling for static 2D plain and MX `tile.matmul`, `tile.matmul_acc`, and `tile.matmul_bias`: pick an L0 tile shape `(m, n, k)` from the active backend's L0 capacities and rewrite the call into a 2-stage pipelined K-loop with per-iter `tile.extract`s. MX rewrites share the same K-only and M/N grids as plain matmul: they use 64-element K alignment, keep the full `[M, K/32]` and `[K/32, N]` scale tiles resident in Mat, reload each scale window as a fresh logical `tile.load` (A5 rejects FP8E8M0 `pto.textract`, mat→mat textract, and boxed non-aligned subviews), then `tile.move` into `LeftScale` / `RightScale`, and pipeline the data and scale transfers as one operand bundle. Capacity checks use the accumulator's physical L0C footprint, not just its logical shape. When that footprint exceeds L0c, the pass M/N-tiles a fresh plain, biased, or MX matmul, or the canonical frontend split-K create/pipeline/store chain, into `[m, n]` output sub-tiles; each MX sub-tile carries matching scale windows from the same Mat-resident scale tiles.

## Overview

Mat-resident matmuls produced upstream by `ConvertTensorToTileOps` + [`FlattenTileNdTo2D`](14-flatten_tile_nd_to_2d.md) carry full `(M, N, K)` operand shapes — almost always larger than the cube unit's L0a/L0b/L0c capacity. This pass picks an L0-fitting `(m, n, k)` and rewrites the matmul into a K-loop whose body extracts `[m, k]` and `[k, n]` slabs into `Left` / `Right` and accumulates into an `Acc`-resident iter-arg. For MX operations it drops the original full scale loads and rebuilds each `ko/32` window as a fresh logical `tile.load`, then `tile.move`s into `LeftScale` / `RightScale` before emitting the matching MX accumulate operation. [`BlockMxScaleTensorViews`](16-block_mx_scale_tensor_views.md) (after AutoTile) physicalizes those per-stage window loads. MX calls whose scale operands are not direct, single-use loads remain unchanged. The loop is marked `ForKind::Pipeline` with `pipeline_stages=2` so the downstream [`LowerPipelineLoops`](31-lower_pipeline_loops.md) pass produces a 2-deep ping-pong on each complete data-and-scale operand bundle. For example, an FP8 `Right[256, 256]` panel that occupies all 64 KiB of A5 L0B becomes two pipelined `Right[128, 256]` panels, allowing two physical L0B stages without changing the public matmul API.
...
6. **M/N tiling (when `m < M` or `n < N`)** — the physical `[M, N]` output Acc footprint overflows L0c.
   ...
   - **peeled** — an `if` with `tile.matmul` / `tile.matmul_mx` for the first K block and `tile.matmul_acc` / `tile.matmul_mx_acc` for later blocks, merged at the branch's phi;
   - **predicated** — a single `init_cond`-bearing `tile.matmul_acc` or `tile.matmul_mx_acc` accumulating into the loop's only iter-arg, with no branch and no phi.
...
| `tile.matmul_mx`, `tile.matmul_mx_acc`, or `tile.matmul_mx_bias` with single-use direct logical scale loads | Same grids as the matching plain op: K-only when the output fits L0c; M/N direct-store or Mat-scratch (and canonical split-K) when it does not. Scale tiles stay in Mat; each stage windows in Mat then moves into LeftScale/RightScale. Vec-left M/N and non-single-use scale loads remain deferred |
| The same call written with a caller-supplied `init_cond` (arity `base_arity+1`) | Also K-tiled: the loop body carries `init_cond and ko == 0`, a lone straight-line full block carries `init_cond` verbatim, and the peeled tail stays at `base_arity` |
| Canonical split-K `create([M,N])` → pipeline (either `matmul[_mx]` first + loop-carried `matmul[_mx]_acc` later, or one predicated `matmul[_mx]_acc(acc, ..., <loop var> == 0)`) → one 2D store, physical output exceeds L0c | M/N-tiled outside the K loop; each `[m,n]` tile completes the full K reduction before it is stored. MX clones four GM loads and narrows the scale windows with the data panels |
...
| Output exceeds L0c but no M/N placement applies — a non-canonical standalone `matmul_acc`, Vec left, a non-matmul-operand consumer, or a chained-matmul scratch whose `[M, N]` exceeds Mat/L1 | Skipped with `PerfHint` (`PH-AT-006`) |
...
| `PH-AT-006` | Output exceeds L0c but no supported M/N placement applies. For plain or MX `tile.matmul_acc`, this specifically means a caller-owned accumulator outside the canonical create/split-K-pipeline/store chain, or an `init_cond`-bearing call inside an otherwise canonical chain whose predicate is not a split-K seed test on the loop's induction variable. It also covers a Vec left operand, an already-Bias-resident `tile.matmul_bias` source that would need N sub-windowing, a result consumed on-chip that is not entirely a matmul operand, or a chained-matmul scratch that exceeds Mat/L1 capacity. The canonical split-K case from issue #2232 does not emit this hint. |
