...
For a canonical split-K chain, the same grid encloses the **source** reduction rather than slicing its final Acc. This output-grid choice uses the single-Acc (`dbC=1`) regime: each generated output tile runs its complete source K reduction and drains before the next tile starts, so this rewrite does not emit the two-Acc `matmul, matmul, drain, drain` schedule required by a `dbC=2` candidate. The ordinary rewrite of calls inside each narrowed reduction still chooses legal inner-K blocking. In issue #2232, the logical INT32 `[16, 1152]` result occupies `32 × 1152 × 4 = 144 KiB` physically on Ascend910B, so it is split along N. Each generated N tile runs all eight source K blocks and stores its result before the next N tile starts.
