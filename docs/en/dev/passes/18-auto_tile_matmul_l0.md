...
2. **Pick L0 tile shape** — call `utils::ChooseL0Tile(cfg)` with the active `BackendHandler`'s `GetL0{a,b,c}CapacityBytes()`, `GetL0FractalAlignment()` / `GetMinL0TileDim()`, `GetL0cMAlignment(accumulator_dtype)`, and `GetL0CostModel()` (L1↔L0 bandwidths + MAD issue overhead), plus per-operand element width (`bytes_a/b/c`) read from the call's result type. Before the call-level search, the pass admits dbC only when an M/N result grid has a semantic placement: a supported direct store or Mat scratch. An arbitrary `tile.matmul_acc` is excluded because its caller-owned full accumulator contains every output tile's initial value and cannot be recycled as two ping-pong slots; the canonical split-K rewrite remains eligible because it controls accumulator creation and the complete reduction. A result without either placement is likewise excluded. This is a chooser design-space constraint, not a late fallback. Biased matmul additionally caps candidate N with `GetBiasCapacityBytes() / bytes_c`; this is a hard auxiliary-SRAM limit, not a roofline coefficient. For a Mat-backed bias reconstructed inside a full-K output grid, the same exhaustive search applies schedule-aware caps: B-stationary keeps one Bias slot, A-stationary and output-stationary with N outer use two, and output-stationary with N in the inner pipeline composes both pipeline memberships and uses four. An already-Bias-resident source is defined outside the grid and always consumes one slot. Candidate L0C legality is `AlignUp(AlignUp(m, box_align_m), l0c_align_m) × AlignUp(n, box_align_n) × bytes_c × dbC <= L0C`; the box alignments default to 1 and are set from the canonical split-K operands' effective Mat layouts when that path will physically pad a window. The boxed M/N extents also gate L0A/L0B. The logical work shape and roofline accounting remain `[m, n]`. `c_read = is_matmul_acc` because `tile.matmul_acc` threads the caller's accumulator through the K-loop's iter-arg (γ_C = 2, doubling the C traffic the model charges). The chooser returns `(m, n, k)` plus the chosen design point — an **exhaustive roofline-`wall` minimum**, not a closed form; see [Cost model & design space](#cost-model--design-space-choosel0tile) below.
...
5. **Build the K-loop** (per output sub-tile — the whole output when K-only, or each `[m, n]` sub-tile when M/N tiling):
   ...
   - **Vec left operand staging** — when the left (A) operand is `Vec`-resident (PV / `score·V`), a single `tile.move(lhs, target_memory=Mat)` is emitted **before** the K-loop or, for M/N tiling, once before the complete output grid. Every Left extract slices from that staged Mat tile (so the extract source is Mat exactly like the QK path); the transfer is never repeated per output tile. Keeping the Vec→Mat crossing a `tile.move` lets [`ExpandMixedKernel`](24-expand_mixed_kernel.md) recognise it (`CollectCVBoundaryMoves` only matches `tile.move`) and lower it to the cross-core `tpop_from_aiv` handshake (which lands the data in Mat). Extracting straight from the Vec tile would instead leave the operand a dangling cross-boundary free variable on the cube side. Because the one staging transfer is identical for every candidate, it is a candidate-independent additive cost and cancels exactly in the chooser's strict dbC-versus-single-C comparison; candidate-dependent L1→L0 traffic remains fully scored.
...
6. **M/N tiling (when `m < M` or `n < N`)** — the physical `[M, N]` output Acc footprint overflows L0c.
   ...
   For a **fresh `tile.matmul` or `tile.matmul_bias` whose result is consumed by exactly one 2D `tile.store(c, base, out)`**, the pass tiles the output into a `ceil(M/m) × ceil(N/n)` grid: for each sub-tile origin `(mi, ni)` it computes the `[m, n]` (partial on the boundary, `min(m, M-mi) × min(n, N-ni)`) sub-tile and emits `tile.store(c_sub, [base_r + mi, base_c + ni], out_prev)`. An N-tiled biased matmul reloads the corresponding `[1, n]` bias window from the defining tensor into Mat and moves it to Bias. If an intervening non-load statement separates the matmul from its consumer store, the chooser instead requires full N so the grid reuses the original bias snapshot; it may still tile M. When **K spans ≥ 2 L0 blocks**, each sub-tile is an independent **pipelined K-loop**. When **`k == K`**, the grid is emitted as nested loops over the divisible interior so [`LowerPipelineLoops`](31-lower_pipeline_loops.md) double-buffers the moving operand. The outer loop owns the stationary panel; output-stationary versus A/B-stationary loop order follows the chooser's design point. The L-shaped partial boundary is peeled into straight-line partial tiles, so `m`/`n` need not divide `M`/`N`. The stores chain the output tensor in SSA form; the final store's result replaces the original store downstream.
   ...
   The following M/N regimes remain **deferred**: an arbitrary standalone `tile.matmul_acc` with a caller-owned accumulator (it does not match the canonical chain above), a biased matmul whose already-`Bias`-resident source would need an unsupported Bias-to-Bias N sub-window, a Mat bias that is not a single-use 2D load with only sibling loads before the matmul, and a result consumed on-chip that is **not** consumed entirely as a matmul operand. One shared placement plan removes dbC before chooser selection and is then consumed by the emitter, so admission cannot drift from realization. If the remaining single-C optimum itself requires unsupported M/N tiling, the ordinary `PH-AT-006` diagnostic remains. The first intervening non-load statement blocks only the N-window reload, not a full-N fold that consumes the original bias value; removing a replaced full load avoids redundant traffic. A result consumed entirely as a matmul operand takes the Mat-scratch placement below. A Vec-left PV matmul uses one grid-wide Vec→Mat stage only when a conservative whole-function Mat bound fits: every existing Mat SSA value at its maximum enclosing pipeline depth, plus the shared cross-core ring at the effective `pl.cross_core_slot(slot_num=N)` depth and largest boundary-slot size and any new Mat scratch, conservatively multiplied by the candidate's enclosing source-pipeline depth. This candidate-independent feasibility cost is checked before M/N emission; unknown capacity or dynamic footprint rejects the route rather than guessing low.
...
`kPipelineDoubleBufferCAttr`. That tag makes `LowerPipelineLoops` stamp the cube
accumulator's `pipeline_membership` (which it otherwise skips, because the cube
serializes MADs) and makes `CanonicalizeIOOrder` rotate it to `stage % 2`, so the
two tiles land in **different L0C slots**. The statement order is unchanged —
`matmul, store, matmul, store` already is the ping-pong, because `store_i` then
runs while `matmul_{i+1}` executes on the other slot.
...
- **Unrolled grid** (`BuildSplitKGrid`, `TryFoldCanonicalSplitKAcc`): the
  ...
  have one uniform physical type. A column-only boundary keeps the accumulator's
  physical row count, so codegen can size the region to the covering full tile
  and emit that narrower use as a zero-offset `pto.subview`. A row boundary
  changes the L0C fractal stride; PTOAS therefore excludes such dbC candidates
  before selection instead of emitting an invalid alias. PyPTO and DSA-RP keep
  native per-tile geometry and admit both boundary directions. Each selected
...
| Output exceeds L0c but no M/N placement applies — non-canonical standalone `matmul_acc`, a non-matmul-operand consumer, or a chained-matmul scratch whose `[M, N]` exceeds Mat/L1 | dbC is excluded before selection; if the single-C optimum still needs M/N tiling, skipped with `PerfHint` (`PH-AT-006`) |
...
| `PH-AT-006` | The single-C choice still requires M/N tiling but no supported placement applies — for `tile.matmul_acc`, this specifically means a caller-owned accumulator outside the canonical create/split-K-pipeline/store chain, or a 4-operand call inside an otherwise canonical chain whose `init_cond` is not a split-K seed test on the loop's induction variable. It also covers an already-Bias-resident `tile.matmul_bias` source that would need N sub-windowing, a result consumed on-chip that is not entirely a matmul operand, or a chained-matmul scratch that exceeds Mat/L1 capacity. Vec-left grids are supported through one hoisted Vec→Mat stage. The canonical split-K case from issue #2232 does not emit this hint. |
