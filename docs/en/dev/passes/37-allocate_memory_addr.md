...
A symbolic relative offset is kept, not dropped: the address becomes the expression `slot_base + member.byte_offset`, which codegen lowers into the tile's runtime address assignment. Such an offset appears when a view's slice offset is a scalar Var — a runtime row, or a constant bound inside an `if` or loop body, where Simplify does not substitute it — as well as for a declared allocation's runtime slot index. Collapsing it onto `slot_base` would make a reshape of that slice read the parent's first rows. Both planners (`PYPTO` and `DSA_RP`) build the address through `MakeAbsoluteMemRefAddress`.
