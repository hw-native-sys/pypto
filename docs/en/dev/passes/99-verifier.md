...
| **AllocTileDominatesUses** | AllocTileDominatesUses | Every use of a `TileType`-with-MemRef variable is dominated by an `alloc_tile` op for its buffer (issue #1956). The checker walks each function body tracking the buffer identities (MemRef base + byte_offset + size) materialized by an `alloc_tile` in the current scope; branch/loop-local materializations do not leak to enclosing scopes (SSA scoping), so an `alloc_tile` declared inside one branch and read from another — the pre-#1956 miscompile under `memory_planner=PTOAS` — is flagged. Keyed by MemRef identity only (not the PYPTO signature split), so it holds under both memory planners: it suffices that *some* handle for the byte-slot dominates the use. **Produced by** [`MaterializeAllocTiles`](41-materialize_alloc_tiles.md) (the last pipeline pass) and listed in `GetVerifiedProperties()`, so `PassPipeline` auto-verifies it once at pipeline end. **Fix**: ensure the pass emits an `alloc_tile` for the buffer at a dominating scope (function head or the point before its first cross-branch use). |
