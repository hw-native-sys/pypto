...
| **HardSyncallOccupancy** | HardSyncallOccupancyValid | A hard (FFTS) `system.syncall` waits for **every** physical core of its `core_type` to reach the barrier, so the enclosing `pl.spmd(N)` launch must fill all of them exactly (`N != required` → error, covering both partial and over-occupancy); a partial launch deadlocks on device (AICore timeout 507018) and leaves it needing a reset. **Produced by** `ExpandMixedKernel` (which resolves each launched kernel's `FunctionType` — the precondition the check needs) and listed in `GetVerifiedProperties()`, so `PassPipeline` auto-verifies it right after that pass. Anchored on each `FunctionType::Spmd` launch function's compile-time-constant `core_num` and its direct callee: standalone **AIV** kernel + `aiv_only` → `required = GetCoreCount(VECTOR)`; standalone **AIC** kernel + `aic_only` → `required = GetCoreCount(CUBE)`; **Group** (mixed kernel) → `required = GetCoreCount(CUBE)` core-groups (one AIC per group, filling all cores) for any barrier `core_type`, checking the hard syncalls in its AIC/AIV sub-kernels (duplicated `mix` reported once). Skipped when the backend is unconfigured, `core_num` is non-constant, or no `pl.spmd` launches the kernel (occupancy is a launch-time property). **Fix**: launch at full occupancy, or use `pl.system.syncall(mode="soft", ...)` (GM-polling) for partial occupancy. |
