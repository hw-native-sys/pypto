...
- **No-alias guard** (op-semantic or compiler-proven): the op that defines the reusing variable may forbid its output from sharing a buffer with one or more of its input operands, because the hardware reads those inputs *while* writing the output — an in-place write would corrupt the op mid-flight. Three sources feed one per-output forbidden-input set (`ForbidAliasCollector`):
  ...
  - compiler call-site evidence — vector AutoTile marks each emitted `tile.cast` because its planner prices a distinct dtype-sized destination. The selected in-tree allocator consumes and strips this private marker. Generic widening casts are also kept distinct because writing wider elements can overtake unread input; legacy narrowing casts retain their established packing contract.
  ...
  | AutoTile `tile.cast`; generic widening `tile.cast` | private call-site evidence; widening check | AutoTile's plan/emit contract prices distinct dtype-sized tiles; generic widening cannot safely overwrite unread narrow elements. Explicit equal-byte zero-copy reinterpretation uses `tile.reinterpret_view` |
...
- cross-dtype reuse — unrelated lifetime-disjoint BF16 and FP32 tiles may share a buffer; AutoTile casts and generic widening casts keep source and destination distinct;
...
- Tests the no-alias guard (`TestForbidOutputAlias` + `TestInplaceOps`), one case per constraint above:
  ...
  - AutoTile `tile.cast` in every dtype direction, plus generic widening casts — output must not alias input
