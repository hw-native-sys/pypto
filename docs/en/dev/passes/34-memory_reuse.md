...
- **No-alias guard** (op-semantic): the op that defines the reusing variable may forbid its output from sharing a buffer with one or more of its input operands, because the hardware reads those inputs *while* writing the output — an in-place write would corrupt the op mid-flight. Three sources feed one per-output forbidden-input set (`ForbidAliasCollector`):
  ...
  - **`tile.cast`** (`not_inplace_safe()`) — PTO `TCVT` is a conversion with distinct source and destination tiles in every dtype direction. An explicit equal-byte `tile.reinterpret_view` is the zero-copy/bitcast opt-in.
  ...
  | `tile.cast` | `not_inplace_safe` | `TCVT` requires distinct source and destination tiles; use `tile.reinterpret_view` for an explicit equal-byte zero-copy reinterpretation |
...
- cross-dtype reuse — unrelated lifetime-disjoint BF16 and FP32 tiles may share a buffer; the source and destination of the **same** `tile.cast` may not;
...
- Tests the no-alias guard (`TestForbidOutputAlias` + `TestInplaceOps`), one case per constraint above:
  ...
  - `tile.cast` — output must not alias its input in any dtype direction
