...
This pass lowers each **1:1** to its tile op (`tensor.aiv_shard` → `tile.aiv_shard`, `tensor.aic_gather` → `tile.aic_gather`), so from here on the IR is byte-identical to what the AUTO `pl.split` path produces via [`LowerAutoVectorSplit`](24-lower_auto_vector_split.md) (pass 24). `ExpandMixedKernel` (pass 25) then folds both into the cross-core `tpush`/`tpop` machinery.
