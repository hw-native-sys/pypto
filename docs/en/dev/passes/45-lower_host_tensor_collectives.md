...
after [`MaterializeCommDomainScopes`](44-materialize_comm_domain_scopes.md), so
...
one pass upstream: [`MaterializeCommDomainScopes`](44-materialize_comm_domain_scopes.md)
...
`for`/`while` loop in `host_orch` — [`MaterializeCommDomainScopes`](44-materialize_comm_domain_scopes.md),
