...
V2C. FP8E8M0 fractal-32 scales never use the NZ TINSERT path. A logical row/row
scale is pushed directly. A logical col/col scale first takes the official,
zero-copy `tile.transpose_view`, producing a row/row carrier with the physically
transposed shape. If producer and consumer logical layouts differ, a real
Vec→Vec `tile.move` converts to the consumer layout before that carrier step.
...
physical box (using `tile.set_validshape` metadata on the carrier) so the ND
insert is contiguous and 32-byte aligned; the consumer still ignores padding
outside its original logical valid region. The carrier's full byte size must
fit the paired pipe's `slot_size`.
