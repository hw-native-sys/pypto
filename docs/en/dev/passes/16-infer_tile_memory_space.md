...
Each InCore function is processed in five phases. All phases run as IR visitors / mutators with O(N log N) complexity in the size of the function body. Phase 4 builds one bottom-up loop inventory; it never rescans a complete nested body for every ancestor loop.
...
This is a conservative first subset of the broader residency behavior requested by issue #2077, not a general tensor-level residency contract. A direct or externally entered InCore function has no caller evidence and therefore declines. When `AutoTileMatmulL0` fans one Mat panel out to multiple K-dependent L0 extracts, the current single-use chain recognizer also declines; the whole-panel GM→Mat load remains inside the outer user loop even though the K-dependent extracts remain correctly inside the inner K loop. Supporting that case requires analyzing GM→Mat panel residency independently from optional Mat→L0 prefix motion. The current caller filter also distinguishes canonical IR roots, but PyPTO does not yet attach a runtime `noalias` contract to distinct external tensor parameters; resolving that contract is required before this draft optimization is merge-ready.
...
- at every direct root-orchestration call site, the candidate `Tensor In` actual root and every writable `Tensor Out` / `Tensor InOut` actual root are known and the candidate root is syntactically distinct from every writable root; the InCore function also does not write that root locally, while unrelated scalars and peer read-only `Tensor In` roots do not participate in this filter;
...
For a caller-backed tensor program, the stationary LHS bridge moves before the user loop, while the N-dependent RHS remains streamed:
...
The internal provenance attribute is omitted above for readability. The transformation currently requires a direct call from a root orchestration function where the candidate `lhs` root is syntactically distinct from the writable `out` root. The peer read-only `rhs` root is irrelevant to that filter. Distinct IR roots alone do not prove that separately supplied runtime buffers cannot alias, so this caller condition remains an open draft-safety item. Without the required direct caller evidence, the original loop-local placement is retained.
