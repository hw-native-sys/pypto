...
| A create under a loop | It is a *fresh* buffer per iteration. Collapsing N buffers into one parameter would make iterations alias, and the cross-task edges that would have to re-serialise them were derived by [`AutoDeriveTaskDependencies`](41-auto_derive_task_dependencies.md), well upstream of here |
...
[`MaterializeRuntimeScopes`](48-materialize_runtime_scopes.md).
...
- [MaterializeRuntimeScopes](48-materialize_runtime_scopes.md) — runs immediately after
