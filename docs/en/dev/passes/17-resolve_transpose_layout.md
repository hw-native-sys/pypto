...
1. **Layout tag.** For canonical trailing-two-dim swaps, `layout` toggles between `ND` and `DN` in IR.
...
The codegen rule above lets *non-transposed* downstream consumers (e.g. `tile.load(..., transpose=False)`) read a DN-tagged `tensor.transpose` result via the explicit-stride path without re-applying the implicit shape swap. For Vec targets on a2a3, a `[1, N]` row load from an explicit-stride DN view cannot use `TLOAD`: row-major Vec tiles violate the `DN2DN` layout requirement, while reshaping the load as `[N, 1]` would change the physical addresses. Codegen keeps the logical row-major `[1, N]` tile and fills it with scalar loads using the recorded strides.
...
**Vec targets on a2a3.** A `pl.transpose` followed by a Vec-target row consumer now lowers without cross-layout `TLOAD(VecTile_RowMajor, GlobalTensor<DN>)`: explicit-stride DN views keep `layout=dn`, and `[1, N]` row consumers use the scalar-load fallback described above. This preserves the transposed-address semantics (`offset_i * stride_i` for each source index) and avoids the a2a3 TLOAD layout restriction. `tile.load(transpose=True)` remains restricted to `target_memory=Mat`; direct `tile.load(transpose=True)` on a `tensor.transpose` result is still rejected as described above.
