...
Fresh SSA vars are **not** by themselves enough to keep the clones in separate buffers: the `F` clones are sequential in program order, so their per-clone tiles have *disjoint* program-order lifetimes — exactly the condition under which `MemoryReuse` would coalesce them into one buffer (defeating the ping-pong). To make stage separation explicit, this pass tags every tile-producing `Call` in clone `k` with a `pipeline_membership` attr recording `(group, stage=k)` (see `include/pypto/ir/transforms/utils/attrs.h`). A tile inside a nested pipeline carries one membership pair per enclosing replicated region, so nested same-core pipelines stay separated at every level. (Historically this separation was an unreliable *side effect* of `CanonicalizeIOOrder` clustering sibling-clone loads to induce lifetime overlap; the explicit attr makes it a hard constraint independent of statement ordering.)

`MemoryReuse` consumes the attr with **role-aware granularity** — forbidding *all* cross-stage reuse (depth = `F`) would need `F` full copies of every intermediate and overflows the on-chip budget on real kernels (e.g. `stage=4` RMSNorm needs `4 × 67 KB > 188 KB` UB). Only **load buffers** genuinely need per-stage privacy so iteration `i+1`'s prefetch overlaps iteration `i`'s compute. So the rule is: block a cross-stage buffer share iff the two tiles are same-group / different-stage **and at least one is a load** (`tile.load` / `tile.read`). Compute intermediates of different stages may still coalesce. The L0 matmul spaces (Left/Right/Acc/Bias) are exempt entirely — their ping-pong is owned by `AutoTileMatmulL0` / `CanonicalizeIOOrder` extract clustering and they are capacity-bound.
