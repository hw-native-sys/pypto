...
The Tensor and Tile shift families (`shl`, `shls`, `shr`, `shrs`) accept only
`INT8`, `UINT8`, `INT16`, `UINT16`, `INT32`, and `UINT32` data. Binary forms
require identical operand dtypes; Tensor operands must have the same shape,
while Tile operands must have equal effective `valid_shape` and may have different
physical shapes. The count operand does not promote the result.
Scalar forms require a signed IR scalar of the same width,
including for unsigned data (`UINT16` data uses an `INT16` count). Python integer
literals and parser placeholders receive that encoding automatically; explicitly
typed expressions keep their dtype and are rejected if incompatible. Shift counts
must be in `[0, bit_width - 1]`. Constant scalar counts are checked during type
deduction without wrapping; callers must keep runtime scalar and tensor counts
in range. Right shift is arithmetic for signed data and logical for unsigned data.
