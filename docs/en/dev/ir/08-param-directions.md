...
| [Wrapper recovery + call sites](#3-wrapper-recovery-and-call-sites-pass-42) | 42 | What is each wrapper's *effective* signature, and what direction does each call-site argument have? |
...
### 3. Wrapper recovery and call sites (pass 42)
...
the check must run after `DeriveCallDirections` (pass 42), and `InitMemRef`
(pass 34) invalidates `SSAForm` with nothing re-establishing it, so no pipeline
position is both after pass 42 and in SSA form. Its buffer lineage is therefore
