...
| - | `tile.mscatter` | Scatter-store tile elements into a GM tensor at per-element indices (`pto.mscatter`): `out[idx[i, j]] = src[i, j]`. `src` dtype ∈ {FP16, FP32, INT16, INT32}; `idx` dtype INT32 (same shape as `src`). |
| **Gather** | `tile.mgather` | Indexed gather-load from a GM table into a fresh VEC tile (`pto.mgather`); the read-side mirror of `tile.mscatter`. Two modes via the `coalesce` attr: **row** (default) `dst[r, j] = mem[idx[r], j]` with `idx` a `[1, R]` (row-major) / `[R, 1]` (col-major) index vector and output `[R, mem_cols]`; **elem** `dst[i, j] = mem[idx[i, j]]` (mem flat-indexed) with output = `idx` shape. `mem`/`dst` dtype ∈ {I8, I16, I32, FP16, BF16, FP32}; `idx` dtype INT32. Usable with loop-local `idx` inside `spmd` / `pl.range`. The index tile's row byte size must be 32-byte aligned (PTOAS `none_box` constraint) — INT32 index width a multiple of 8; pad to the next aligned width + `valid_shapes` to gather a non-multiple-of-8 count. |
