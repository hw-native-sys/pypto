...
parser 会在能够证明违约时报错——当操作数是由前序 `pl.submit(...)` 结果绑定的变量时，其
producer TaskId 必须出现在 `deps=` 中。以下两种情况无法证明，放行并由作者自行保证：操作数
没有可追踪的 producer（例如函数参数），以及 `deps=` 条目是 `Array[N, TASK_ID]`（未逐个列出
其 producer）。
