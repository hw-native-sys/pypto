...
`predicate=pl.dispatch_pred(tensor, indices, op, target)`。调度器在 **dispatch 点**
求值 `tensor[indices] <op> target` —— 此时该 task 的依赖已满足，值一定是最新的，
...
| 参数 | 含义 | 约束 |
| --- | ------- | ---------- |
| `tensor` | dispatch 点读取的操作数张量 | 必须是 tensor |
| `indices` | 定位 `tensor` 中一个元素的下标 | **list 字面量**；每个元素是 `ConstInt` 或循环 `Var`；非空 |
| `op` | 比较算子拼写 | **字符串字面量**，取 `==` `!=` `>` `<` `>=` `<=` 之一 |
...
点读到的才是最新值。当前仅在构造时做结构性校验（operand 存在且为 tensor、indices 非空、
`op` 在范围内）；数据流“producer ∈ deps”检查尚未落地，故需作者自行保证 `deps=` 写对。

**表达力**固定为 `tensor[indices] OP const` —— 单个比较，与运行时单比较的
`DispatchPredicate` 对齐。多条件 / 布尔组合 / 算术均无法表达；请在前序 kernel 里把它们
归约成一个 gate 值,再对该值做谓词。
