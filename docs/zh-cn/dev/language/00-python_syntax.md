...
- **在 `pl.spmd` 上，`deps=` 需要 `as tid` 形式。** `deps=` 只在
  `with pl.spmd(...) as tid:` 上被接受。因此，若谓词读取的张量由同一函数内的其他任务产出，
  就必须用该形式；普通 `with` 与 `for` 形式只能对没有函数内 producer 的张量（通常是函数
  参数）加谓词——这种情况契约检查会放行。（`pl.at` 的所有形式都接受 `deps=`，不受此限制。）
...
- **不能嵌套在 `pl.cluster()` / `pl.spmd()` / 另一个 `pl.at` 内。** 这几种情况下内层
  dispatch 会被折叠进外层 Group / Spmd wrapper（或外层 kernel 体），codegen 只会从
  **外层** call 发出 `set_predicate`。同样在解析期拒绝。

与 `pl.spmd` 不同，`pl.at` 的所有形式都接受 `deps=`，所以对函数内产出的张量加谓词时，
