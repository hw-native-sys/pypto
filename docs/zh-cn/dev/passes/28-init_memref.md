...
某个 tile 进入 InitMemRef 时其 `TileType` 上可能已带 MemRef —— 即用户的 `pl.MemRef(addr, size, id)` 注解（InitMemRef 之前没有任何 pass 创建 MemRef，因此存在 `memref_` 只能来自用户 pin）。当一个可 pin 的空间 —— `Vec`、`Mat`，或 L0 matmul 空间（`Left`/`Right`/`Acc`/`Bias`）—— 中存在任意一个 pin 时，该空间进入**整空间手动**模式：

- pinned tile 被 honor：InitMemRef 按 `id` intern 其 base Ptr（重命名为 `__pinned__<id>` 前缀，使 MemoryReuse / AllocateMemoryAddr 无需 IR 节点标志即可识别），保留用户的字节地址，并校验声明的 buffer 足够大。
- 该空间内其余每个新建 tile 也必须被 pin；未标注者为硬错误（它会是编译器临时 tile，本特性不支持）。

完整特性说明与「不复用 / 重叠」规则见 [MemoryReuse](31-memory_reuse.md)。
