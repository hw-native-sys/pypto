...
## 多缓冲 slot 重定向（PTOAS planner）

当 `LowerPipelineLoops` 在 PTOAS planner 下把 tile carry 降级为多缓冲时（参见 [LowerPipelineLoops](25-lower_pipeline_loops.md)），InitMemRef 为每个 `tile.multi_get` slot 分配各自独立的 MemRef（一块独立物理 slot）。随后，被打上 `kMultiBufferAliasSlotAttr`（slot 变量名）标记的 carry producer 通过 `ShareMemRefFrom` 重定向到该 slot 的 MemRef，使 codegen 让 producer 写 `outs(<slot>)` —— carry 物理上落在轮转 slot 中。slot 按名字记录（`name_hint_` 能在中间 pass 可能的变量克隆中保留），且总是在引用它的 producer 之前被物化。
