...
对于消费侧跨核 tile，`tfree` 修复步骤会保留输入进来的 `tpop` / user 顺序，只在需要时修正匹配
`tfree` 的位置。若缺少 `tfree`，则会在被 pop tile 的最后一次活跃使用之后自动补齐；若边界
`tpop` 经过了重映射，生成的 `tfree` 也会继续绑定到规范化后的 tile 值。
...
这样，常见错误（缺少 `initialize_pipe`、缺少 `reserve_buffer` / `import_peer_buffer`、缺少 `tfree`、非静态 tile 形状）会在 `ExpandMixedKernel` 之后立即报出，而不是拖到 PTO codegen / `ptoas` 阶段。
...
| 自动生成 tfree 链 | 消费侧拆分内核会在 IR 层于最后一次活跃使用之后释放每个 `tpop` 得到的槽位，同时保留交错的 `tpop` / user 顺序 |
