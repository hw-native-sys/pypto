...
- Phase 1：对 `peer` 从 `0` 到 `NR-1`，`pld.tile.remote_store(stage_tile, target, peer, [my_rank, 0])` — 将本 rank 的 chunk 推送到每个 peer 窗口的第 `my_rank` 行。自推送 (`peer == my_rank`) 通过 HCCL 恒等映射实现
