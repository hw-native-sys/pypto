...
`AutoDeriveTaskDependencies` 在 runtime scope 内保守推导 task-to-task
依赖边。它运行在
...
本 pass 在默认 pipeline 中会分析 MANUAL 区域。AUTO 区域不同：只有编译期开关
`analyze_auto_scopes_for_deps` 开启时才会分析。手写的 AUTO `RuntimeScopeStmt`
...
deps，pass 会从整个 enclosing `RuntimeScopeStmt` 中剥离任何部分编译器推导 deps，
并以 `manual=false` 返回。已实现的 fallback 触发条件包括：
...
这样整段区域都会回到 runtime OverlapMap/TensorMap tracking，而不会在 scope
边界混用部分 compiler deps 与 runtime 状态。

这个 fallback 也适用于用户手写的 MANUAL scope。它是正确性 fallback，不是性能
保证：如果编译器无法把所有必需依赖安全编码成固定 TaskId 边，用户写的
`pl.manual_scope()` 也可能最终按 AUTO 编译。用户手写的 `deps=[...]` 仍会保留，
缺失的保守排序由 runtime tracking 补上。

## 默认路径变化

- MANUAL scope 默认会被分析，并可能新增 `compiler_manual_dep_edges`，这些边会
  下沉为 `Arg::set_dependencies(...)`。这些边是保守的，可能增加本 pass 接入前
  不存在的排序。
