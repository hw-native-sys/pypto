...
部分有效的"自由"行维:它之前的每个行维都是单位维（`1`,其下标被钉在 0）,它之后的每个行维都是完全有效的
（`valid == shape`）。上面的 strip-mine 情形满足它 —— 前导的 `1` 钉住其下标,中间的 `CHUNK` 维是自由维。
若某个*中间*维在非单位外层维之下仍是部分有效（例如物理 `[2, 4, 8]` 配 valid `[2, 2, 8]`）,则会得到跨步
（strided）区域,Pass 以 `ValueError` 拒绝;否则乘积折叠会悄悄把 batch-0 的填充标为有效、把真实的 batch-1
数据标为无效。

**空区域豁免。** *空*的有效区 —— 任意维可证为 `0`,例如 `pl.create_tile(valid_shape=[0, 0, 0])` 累加器
（D2 的"尚无有效数据"）—— 表示空集,它平凡地是一个长度为 0 的连续前缀。上面的钉住检查关心哪些维被*钉住*
（`valid == 1`）,否则会把 `0` 维误判为部分有效的中间维并给出误导性的报错,因此当任意维为 `0` 时跳过该检查。
