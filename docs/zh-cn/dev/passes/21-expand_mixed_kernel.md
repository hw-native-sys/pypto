...
- `slot_num`：单向 / 双向默认均为 `kDefaultRingSlots`（`2`）。作者可以在外层 `pl.at(level=pl.Level.CORE_GROUP, optimizations=[...])` 上通过 `pl.split(MODE, ring_slots=N)` 按 scope 覆盖默认值；该覆盖在 parser 中写入 `InCoreScopeStmt.attrs`，再由 `OutlineIncoreScopes` 透传至外联函数的 `attrs_["ring_slots"]` 上，本 Pass 在此处消费。当 scope 受时延约束且 Vec/UB 仍有余量时可设为更大值（历史默认值为单向 8、双向 4）。
