...
### 奇数 AUTO Vector→Cube 边界

带 `odd_split_gm_sync` 标记的 AUTO `tile.aic_gather` 不能使用等长半块的
`tpush_to_aic` / `tpop_from_aiv` 传输。本 pass 为每个此类边界创建一个 L0 对齐的
隐藏 GM 传输张量，并按下表下降：
...
两个 AIV lane 因而把逻辑 floor/ceil shard 写入同一个 GM 张量的相邻区域。每个边界
获得不同的空闲 event id：先排除显式 `sync_set` / `sync_wait` 已用的 id，再从 `13`
向下分配。在 A2/A3 上，本 pass 还会创建 `system.set_ffts` 所需的 256 元素 INT64
FFTS workspace，并传给两条 lane。

隐藏传输张量/workspace 是 `NO_DEP` 内核参数。独立 Group 在本地创建它们；当外层
wrapper 调用 Group 时，由调用者创建，内部 Group 则把它们暴露为参数，从而保持
orchestration codegen 的 wrapper 参数映射约束。
...
该通用检测器仅对 **Cube→Vector** 方向（cube `tile.store` → vector `tile.load`）插入栅栏。AIC 侧的 `tpush` 直接发送被存储的 tile（与常规边界 C2V 推送在两种后端上的行为一致），AIV 侧 `tpop` 落在 `Vec`。通用的反向 Vector→Cube GM 交换需要 fractal 布局适配，因此保持不加栅栏。上文编译器生成的奇数 AUTO V→C 路径是独立机制：它有意把逻辑 shard 写入 GM，并在 `sync_set` / `sync_wait` 后重新 load 到 `Mat`。
...
阶段 2 — 展开每个 InCore 函数 F：
  ...
  2b. 为每个 odd_split_gm_sync V→C 边界分配对齐的隐藏 GM 张量和不同的空闲
      event id；A2/A3 上同时分配 FFTS workspace
  ...
  4. 构建 AIC 函数体：保留 CUBE + SHARED 语句，删除 VECTOR，递归处理 MIXED 循环
     ...
     - 对于普通 Vector→Cube：生成 dest_var = tpop_from_aiv()，携带 fractal TileView
     - 对于奇数 AUTO Vector→Cube：等待对应 event，再把隐藏 GM 张量 load 到 Mat
  5. 构建 AIV 函数体：对称（保留 VECTOR + SHARED，删除 CUBE）
     - 对于边界移动（Cube→Vector）：生成 dest_var = tpop_from_aic()，携带 fractal TileView
     - 对于普通 Vector→Cube：生成 tile.move 适配 fractal 布局，然后 tpush_to_aic(adapted_tile)
     - 对于奇数 AUTO Vector→Cube：把 floor/ceil lane shard store 到隐藏 GM，再 sync_set
 ...
 12. 创建 AIC 函数（无返回值）和 AIV 函数（原始返回值），把奇数边界的隐藏
     transfer/workspace 作为 NO_DEP 参数追加
...
**测试**：`tests/ut/ir/transforms/test_expand_mixed_kernel_a2a3.py`、
`tests/ut/ir/transforms/test_expand_mixed_kernel_a5.py`、
`tests/ut/ir/transforms/test_expand_mixed_kernel_split_aiv.py`，以及上板 `pl.jit`
用例 `tests/st/runtime/cross_core/test_auto_split_odd.py`。
