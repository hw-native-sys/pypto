...
7. [`LowerAutoVectorSplit`](20-lower_auto_vector_split.md)（在用自动拆分下降路径；在 ExpandMixedKernel 之前把 AUTO `pl.split` 混合 InCore 函数转换为显式 `split_aiv` 形态）
8. `ExpandMixedKernel`
9. [`InjectGMPipeBuffer`](22-inject_gm_pipe_buffer.md)
10. [`SplitVectorKernel`](23-split_vector_kernel.md)（仅为 split_aiv 函数打属性 + 处理无拆分双 AIV 路径）
