...
针对右操作数为 Mat（左操作数为 Mat 或 Vec）的 `tile.matmul` / `tile.matmul_acc` 进行 L0 切分：从当前 backend 的 L0 容量中挑选 L0 tile 形状 `(m, n, k)`，并用 `tile.extract` 把 Mat 面板送入 Left/Right。M/N 切分既用于输出超过 L0c 的情况，也用于 Vec-left PV 的 2 阶段 Right 面板超过 L0B 的情况。
...
**K 切分 vs M/N 切分。** 当 chooser 返回 `m == M` 且 `n == N` 时，只切分 K 维。当返回 `m < M` 或 `n < N` 时，本 pass 计算 `ceil(M/m) × ceil(N/n)` 的 `[m, n]` 子块网格（边界处为部分块）。通常这是为了让 Acc 放进 L0c；对于 Vec-left PV，若完整 A 面板可放进 L0A、但 2 阶段 `[K,N]` Right 面板超过 L0B，则再次调用 `ChooseL0Tile`，约束为 N-only/full-K/output-stationary（`m=M`、`k=K`），同时保留 planner 选择的 dbC gate。A2/A3 上 `[32,128] @ [128,512]` BF16 会得到 `[128,128]` Right tile：每阶段 32 KiB，两阶段正好 64 KiB。
...
5. **构造 K-loop**（针对一个输出子块——K 切分时即整个输出，M/N 切分时为每个 `[m, n]` 子块）：
   ...
   - **Vec 左操作数预存（staging）** —— 当左（A）操作数为 `Vec`（PV / `score·V`）时，在整个 K-loop 或 M/N 网格**之前**只插入一次 `tile.move(lhs, target_memory=Mat)`，所有 Left `tile.extract` 都从这个 Mat tile 切片。把 Vec→Mat 这一跨界保持为 `tile.move`，可让 [`ExpandMixedKernel`](19-expand_mixed_kernel.md) 识别它并 lower 成跨核 `tpop_from_aiv` 握手。
...
对于 Vec-left PV `[32,128] @ [128,512]` BF16，左 tile 只 move 到 Mat 一次，N-loop 每次抽取 `[128,128]` Right 面板。若原结果由 `slice[0:16,:]` 与 `slice[16:32,:]` 消费，则每个 `[32,128]` Acc 子块会产生两个 `[16,128]` slice，并在当前 N 偏移处 store；不会创建完整的 `[32,512]` Vec scratch。
...
| Vec-left PV 的 2 阶段完整 Right 面板超过 L0B，且结果由一个完整 store 或有限个互斥、常量、full-N 行 slice-store 消费 | Vec 左 tile 只预存一次；`ChooseL0Tile` 选择 N-only/full-K/output-stationary 网格，并把每个 store 分布到所有 N 子块 |
...
| 已选择 M/N 切分但放置不适用——`matmul_acc`、重叠/动态/不完整的 PV slice、其它非矩阵乘消费者、或 `[M, N]` 超过 Mat/L1 的链式 scratch | 以 `PerfHint`（`PH-AT-006`）跳过 |
...
| `PH-AT-006` | Chooser 已选择 M/N 切分，但没有适用的放置：`tile.matmul_acc`；重叠、动态、partial-N、未串联或其它不支持的 PV slice 消费；或结果在片上被非矩阵乘操作数消费。完全作为矩阵乘操作数消费时走 **Mat-scratch**，除非 scratch 超过 Mat/L1 容量。 |
