...
`ReduceOp.Sum`，并要求 signal 是 INT32 tensor。Mesh allreduce（`mode="mesh"`）
的 signal 形状为 rank-1 `[world_size]` 或 rank-2 `[world_size, 1]`；ring
allreduce（`mode="ring"`）的 signal 为 rank-2，形状为 `[2 * (NR - 1) + 1, NR]`，其
`shape[0]` 在 signal 两个维度均为编译期常量时必须恰好等于 `2 * (NR - 1) + 1`（否则至
少 `2 * (NR - 1) + 1`）。当参与设备数静态可知时，signal 的静态容量必须足够。ring allreduce 还会检查 `numel(src) // NR` 是否整除（对应
ring schedule 的 chunk 分区）。
