...
allreduce（`mode="ring"`）的 signal 为 rank-2，形状为 `[2 * (NR - 1), NR]`，其
`shape[0]` 在 signal 两个维度均为编译期常量时必须恰好等于 `2 * (NR - 1)`（否则至
少 `2 * (NR - 1)`）。当参与设备数静态可知时，signal 的静态容量必须足够。ring allreduce 还会检查 `numel(src) // NR` 是否整除（对应
