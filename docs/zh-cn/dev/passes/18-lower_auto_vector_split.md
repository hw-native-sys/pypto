...
2. **早期显式下降（本 pass）** —— 在 `ExpandMixedKernel` 之前，把 AUTO `pl.split`
   ...
   普通算子驱动边界会把 `tile.aiv_shard` / `tile.aic_gather` 折叠为带拆分标记的
   `tpush`/`tpop`。奇数 AUTO V→C 边界携带内部标记，并走下文所述的 GM +
   `sync_set`/`sync_wait` 路径。
...
函数级 AUTO 路径支持 **Vector→Cube** 边界上的静态奇数拆分轴。逻辑长度为 `N` 时，
两个 AIV lane 使用统一的 `ceil(N/2)` 物理 tile 类型，但有效长度分别为
`floor(N/2)` 与 `ceil(N/2)`，lane 1 的起始偏移为 `floor(N/2)`。因此
LEFT_RIGHT 的 `255` 会得到有效宽度 `127`、`128`，偏移 `0`、`127`，不会丢失或
重叠有效元素。

生成的 `tile.aic_gather` 带 `odd_split_gm_sync` 标记。其物理 Cube box 的每个维度
都补齐到后端 L0 对齐，而 `TileView.valid_shape` 保留原始逻辑形状。随后
`ExpandMixedKernel` 把两个不等长 AIV shard 写入同一个隐藏 GM 张量，并用
`system.sync_set` / `system.sync_wait` 同步 AIC load；该边界不使用
`tpush_to_aic` / `tpop_from_aiv`。

该能力有意只对 AUTO 整函数路径开放。显式 `pl.split_aiv` 区域和旧传输路径仍要求
静态轴为偶数。奇数 **Cube→Vector** 边界同样会被拒绝，除非更早的奇数 V→C 边界
已经产生带奇数逻辑 valid shape 的偶数补齐物理 box。
...
3. LowerStmts 遍历扁平函数体：
   ...
   边界 tile.move（ClassifyMoveDirection）：
     ...
     VECTOR_TO_CUBE —— 插入
       ...
       V->C 边界据此命名其合成的 tpop）。对于静态奇数 AUTO 轴，使用补齐后的物理
       Cube 类型、保留逻辑 valid shape，并给 gather 打 `odd_split_gm_sync` 标记，
       交由 GM 会合路径下降。
...
**测试**：`tests/ut/ir/transforms/test_lower_auto_vector_split.py`、
`tests/ut/ir/transforms/test_expand_mixed_kernel_a2a3.py` 中的奇数 GM 边界覆盖、
`tests/st/runtime/cross_core/test_auto_split_odd.py` 中的上板 `pl.jit` 用例，以及
...
- [`ExpandMixedKernel`](19-expand_mixed_kernel.md) —— 紧邻其后运行；把
  普通 `tile.aiv_shard` / `tile.aic_gather` 折叠为带拆分标记的 `tpush`/`tpop`，
  并把奇数 AUTO V→C gather 下降为 GM + 手动事件同步。
