...
| **AllocTileDominatesUses** | AllocTileDominatesUses | 每一个 `TileType`-带-MemRef 变量的使用都必须由其 buffer 的一个 `alloc_tile` op 支配（issue #1956）。检查器遍历每个函数体，追踪当前作用域中由 `alloc_tile` 物化的 buffer 身份（MemRef base + byte_offset + size）；分支/循环局部的物化不会泄漏到外层作用域（SSA 作用域），因此一个在某个分支内声明、却从另一个分支读取的 `alloc_tile`——即 `memory_planner=PTOAS` 下 #1956 之前的错误编译——会被标记。仅按 MemRef 身份作键（而非 PYPTO 的签名拆分），因此在两种 memory planner 下都成立：只要该字节槽位的*某个* handle 支配该使用即可。**由** [`MaterializeAllocTiles`](41-materialize_alloc_tiles.md)（最后一个流水线 pass）**产生**并列入 `GetVerifiedProperties()`，故 `PassPipeline` 在流水线结束时自动验证一次。**修复方式**：确保该 pass 在一个支配作用域（函数头部，或该 buffer 首次跨分支使用之前的位置）为该 buffer emit 一个 `alloc_tile`。 |
