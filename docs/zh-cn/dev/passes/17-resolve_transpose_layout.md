...
1. **Layout tag。** 对最后两维互换的标准情形，IR 中的 `layout` 在 `ND` 与 `DN` 之间切换。
...
上述 codegen 规则让 *非转置* 的下游消费者（例如 `tile.load(..., transpose=False)`）能够通过显式 strides 路径正确读取 `tensor.transpose` 的 DN 结果，不会再叠加一次"末两维 swap"造成的双重转置。对 a2a3 上的 Vec 目标，从显式 stride DN 视图读取 `[1, N]` 行时不能使用 `TLOAD`：行主序 Vec tile 不满足 `DN2DN` 布局要求，而把 load 改写成 `[N, 1]` 会改变真实物理地址。Codegen 会保留逻辑上的行主序 `[1, N]` tile，并用记录的 strides 逐元素 scalar load 填充。
...
**a2a3 上的 Vec 目标。** `pl.transpose` 后接 Vec 行消费者时，现在不会再 lower 出跨布局 `TLOAD(VecTile_RowMajor, GlobalTensor<DN>)`：显式 stride 的 DN 视图保持 `layout=dn`，`[1, N]` 行消费者使用上述 scalar-load fallback。这会保留转置后的寻址语义（每个源下标按 `offset_i * stride_i` 计算），同时避开 a2a3 的 TLOAD 布局限制。`tile.load(transpose=True)` 仍限制为 `target_memory=Mat`；直接对 `tensor.transpose` 结果做 `tile.load(transpose=True)` 仍如上文所述被拒绝。
