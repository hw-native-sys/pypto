...
[`MaterializeInplaceAliases`](30-materialize_inplace_aliases.md) 之前运行。
它总是运行；只有与规划器相关的机会别名阶段是条件执行的。
...
- 在 `PTOAS` 下，后续的 `MaterializeInplaceAliases` 只编码合法的
  `dst == dead-src` 操作边界，之后仍由 PTOAS 完成全局装箱。
