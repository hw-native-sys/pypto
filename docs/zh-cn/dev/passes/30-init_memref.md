...
某个 tile 进入 InitMemRef 时其 `TileType` 上可能已带 MemRef —— 即用户的 `pl.MemRef(addr, size, id)` 注解（InitMemRef 之前没有任何 pass 创建 MemRef，因此存在 `memref_` 只能来自用户 pin）。当一个可 pin 的空间（`Vec`/`Mat`）中存在任意一个 pin 时，该空间进入**整空间手动**模式：
