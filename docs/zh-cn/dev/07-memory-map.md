...
<kbd>Esc</kbd>、或点击标记上的 `×` 即可取消。每张函数卡片各自维护自己的选中状态。
...
收起时，行号栏吸附在左边缘，panel 横向滚动时保持可见。展开后，该列宽度等于最长的一行
IR 并**取消吸附**——整个面板横向滚过完整源码再滚到 panel，因为继续吸附恰好会挡住刚展开
的内容。绘图面板高度上限为视口的 78 %，这样长函数的两个方向滚动条都够得着。
