...
- **端到端（ST）**：`tests/st/distributed/test_l3_allreduce.py`（mesh allreduce；
  ...
  ``--device=0,1,2,3`` 或 ``--device=0-3``）、`test_l3_allgather.py`（动态 NR，P=2/P=4）、
  `test_l3_reduce_scatter.py`（动态 NR，P=2/P=4）、
  `test_l3_broadcast.py`（动态 NR，P=2/P=4）、`test_l3_gemm.py`、
