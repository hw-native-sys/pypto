...
| `slice` | `(input: T, shape: Sequence[IntLike], offset: Sequence[IntLike], valid_shape: Sequence[IntLike] \| None = None, drop_dims: Sequence[int] \| None = None, *, clamp: bool = False) -> T` | 带偏移的全 rank 切片。`drop_dims` 在证明相交后的有效 extent 为 1 后，显式删除所列静态单位轴。`valid_shape` 收窄结果；仅限关键字的 `clamp=True` 通过在 `offset` 处把窗口夹到源有效区来推导不规则尾块（从不加宽，并与显式 `valid_shape` 求交）。tile 切片的有效传输必须落在源分配范围内 |
| `valid_dim` | `(input: T, axis: int) -> Scalar` | 编译期查询 `valid_shape[axis]`，返回 `Scalar[INDEX]`（未设置时按 D2 返回物理范围）；`axis` 必须在 `[0, rank)`。让内核消费不规则范围，例如把行和除以有效列数 |
...
| `create` / `create_tile` | `(shape: Sequence[IntLike], dtype: DataType, target_memory: Mem, valid_shape: Sequence[IntLike] \| None = None) -> Tile` | 在指定内存空间创建 tile（tile-only，对应 `pl.tile.create`）。`valid_shape` 收窄初始有效区 —— `[0, 0]` 造出空累加器供 `assemble` 增长；未设置即完全有效（D2） |

`shape`、`offset` 和 `valid_shape` 始终保持全 rank。标量索引通过在 `shape`
中保留物理单位 extent，并把该轴列入 `drop_dims` 来显式表达；空的或无法证明为
全有效的单位轴不能删除。Tile 结果保持后端要求的最小物理 rank 2。所有 slice API
的 `clamp` 都只能按关键字传入。底层 `pypto.ir.op.tensor.slice` 与
`pypto.ir.op.tile.slice` 还依次暴露位置参数 `drop_dims`、`pad_value` 和 `span`；
`clamp` 仍必须写成关键字。
...
| `slice` | `(tensor: Tensor, shape: Sequence[IntLike], offset: Sequence[IntLike], valid_shape=None, drop_dims=None, pad_value=None, *, clamp=False) -> Tensor` | 全 rank 切片。语法糖：`A[0:16, :]`。`drop_dims`、`valid_shape` 与 `clamp` 同统一的 `pl.slice`；`pad_value` 决定有效区外的读取值。越界的裸切片若无 `clamp` / `valid_shape` / `pad_value`，则为静态越界错误 |
...
| 名称 | 签名 | 说明 |
| ---- | ---- | ---- |
| `load` | `(tensor: Tensor, offsets: Sequence[IntLike], shapes: Sequence[IntLike], valid_shapes: Sequence[IntLike] \| None = None, target_memory: Mem = Mem.Vec) -> Tile` | DDR → 片上 tile。`offsets` 和 `shapes` 均使用源 tensor 的坐标系。`valid_shapes`（每维 `<=` `shapes`；缺省为 `shapes`）设置 tile 的 `valid_shape` —— 不规则尾块的运行时有效子区域（可为 `Scalar[INDEX]`）—— 并与源 tensor 自身的有效区求**交集**。转置 matmul 操作数请对 load 结果叠加 `transpose_view`。 |
...
| `create` | `(shape: Sequence[IntLike], dtype: DataType, target_memory: Mem = Mem.Vec, valid_shape: Sequence[IntLike] \| None = None) -> Tile` | 在指定内存空间创建 tile。`valid_shape` 收窄初始有效区（`[0, 0]` = 空累加器）；未设置即完全有效（D2） |
...
| 名称 | 签名 | 说明 |
| ---- | ---- | ---- |
| `slice` | `(tile: Tile, shape: Sequence[IntLike], offset: Sequence[IntLike], valid_shape=None, drop_dims=None, pad_value=None, *, clamp=False) -> Tile` | 全 rank 切片。语法糖：`A[0:16, :]`。`drop_dims`、`valid_shape`、`pad_value` 与 `clamp` 同 Tensor 版本；有效传输必须落在源分配范围内，而 `clamp=True` 或可放入的显式尾块可以留下物理无效后缀。删除轴后物理结果不会低于 rank 2。PTO 将其降级为纯 subview，因此 codegen 会拒绝非空的切片 `pad_value`；应对切片结果调用 `pl.fillpad` |
