...
| `slice` | `(input: T, shape: Sequence[IntLike], offset: Sequence[IntLike], valid_shape: Sequence[IntLike] \| None = None, clamp: bool = False) -> T` | 带偏移的切片。`valid_shape` 收窄结果的有效区；`clamp=True` **推导**不规则尾块 —— 在 `offset` 处把窗口夹到源的有效区（从不加宽，与显式 `valid_shape` 求交集），并抑制静态越界检查 |
...
| `slice` | `(tensor: Tensor, shape: Sequence[IntLike], offset: Sequence[IntLike], valid_shape=None, clamp=False) -> Tensor` | 切片。语法糖：`A[0:16, :]`。`valid_shape` / `clamp` 同统一的 `pl.slice`（越界的裸切片若无 `clamp` / `valid_shape` / `pad_value` 则为静态越界错误） |
...
| 名称 | 签名 | 说明 |
| ---- | ---- | ---- |
| `slice` | `(tile: Tile, shape: Sequence[IntLike], offset: Sequence[IntLike], valid_shape=None, clamp=False) -> Tile` | 切片（最多 2D）。语法糖：`A[0:16, :]`。`valid_shape` / `clamp` 同统一的 `pl.slice` |
