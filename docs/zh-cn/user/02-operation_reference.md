...
| `bitcast` | `(input: T, dtype: DataType, *, strict: bool = True) -> T` | 零拷贝元素类型重新解释，复用同一段字节。`strict=True`（默认）要求位宽相等；`strict=False` 额外允许窄化（仅限 row_major / ND 源）。加宽与同 dtype 均拒绝。数值转换请用 `cast` |
...
| `bitcast` | `(tensor: Tensor, dtype: DataType, *, strict: bool = True) -> Tensor` | 零拷贝元素类型重新解释。**仅支持 in-core** —— 编排层不支持。位宽规则见统一算子表中的 `bitcast` |
...
| `bitcast` | `(tile: Tile, dtype: DataType, *, strict: bool = True) -> Tile` | 零拷贝元素类型重新解释，与源 buffer 同址，不产生任何指令。位宽规则见统一算子表中的 `bitcast` |
