...
类型兼容且生命周期先后相接的声明，只有在共享分配分析也允许别名时才能复用同一物理区域：
定义操作的 `forbid_alias` 输入和目标特有的 hazard 都要与曾占用该区域的每个逻辑分配核对。
槽位 handle 的身份还包含静态有效尺寸，因此同一槽位、相同物理形状但有效尺寸不同的边界
视图，会得到不同的 `pto.subview` 结果。
