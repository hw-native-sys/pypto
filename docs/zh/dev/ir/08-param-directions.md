...
| [Wrapper 恢复与调用点](#3-wrapper-恢复与调用点pass-42) | 42 | 每个 wrapper 的**有效**签名是什么？每个调用点实参的方向是什么？ |
...
`DeriveCallDirections`（pass 42）之后运行，而 `InitMemRef`（pass 34）作废了
`SSAForm` 且此后无人重建，因此流水线中不存在既在 pass 42 之后、又处于 SSA 形式的
