...
只有 MAD 本身无法寻址的窗口才会被打包。列数不超过 16 的累加器只有一个 L0C 块列；
窗口行数按 16 向上取整后等于父累加器行数时，写入步长恰好就是父累加器的步长 ——
pto-isa 的 `MadAccStrideCompatible` 接受这两种形态。这类链原样通过，下面的要求对
它们一律不适用：把硬件本就接受的链纳入打包，只会让原本可用的 kernel 落入下列拒绝
条件。
