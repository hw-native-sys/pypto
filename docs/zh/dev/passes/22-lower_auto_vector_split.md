...
任何 `SplitAivScopeStmt` 会到达 `ExpandMixedKernel`（pass 23）或 codegen。
...
[`ExpandMixedKernel`](23-expand_mixed_kernel.md) 会把每条 `SHARED` 语句复制到**两条**
...
其生命周期与 `pipeline_stages` 相同（[`LowerPipelineLoops`](30-lower_pipeline_loops.md) →
[`CanonicalizeIOOrder`](31-canonicalize_io_order.md)）。
...
- **任务并行体**（`None`）：**没有拆分轴**——两个 AIV lane 都运行**完整**区域体，由作者通过
  ...
  **原样透传区域体**（不折半、不本地化偏移、不注入 `subblock_idx`；作者的
  ...
  都是全宽。该函数仍会被标记 `split_aiv`，因此下游 [`ExpandMixedKernel`](23-expand_mixed_kernel.md) /
...
- **`tile.tpush_*` / `tile.tpop_*` / `system.tfree_*` 携带 CODE**（`0`..`4`），由
  [ExpandMixedKernel](23-expand_mixed_kernel.md) 在把边界折叠成传输对时，根据该 mode
...
[ExpandMixedKernel](23-expand_mixed_kernel.md) 用同一分区推导传输 code。在均分的函数体里
