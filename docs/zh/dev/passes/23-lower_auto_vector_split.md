...
检查诊断分别记录全宽算子名（`full_width_vec_ops`）与循环携带变量名（`carry_mismatches`）。携带值不一致时，说明入口的 shard 事实在回边丢失，并提示保留按 lane 的 yield；算子未本地化时，报告算子名，并提示使用按 lane 的操作数或本地化读取地址。文案适用于显式边界区域、隐式下降区域和 AUTO body。两类问题同时存在时，优先报告携带值不一致。
