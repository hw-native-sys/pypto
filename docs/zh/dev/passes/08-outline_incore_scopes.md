...
对于生成的 InCore 函数，如果每个返回 tensor 都能唯一解析到不同的已捕获
`Out`/`InOut` 参数，外提器会按捕获参数顺序同步排列返回值、返回类型和调用方投影。
所有输出共用一次 body 索引；只有 `IfStmt` 的每个分支都解析到同一个参数时，分支结果
才被视为同一 buffer。只要存在新分配、scalar、歧义、无法追踪或重复映射的输出，整组
生成契约就保留原顺序。因此，在能够安全规范化时，SSA 命名顺序和定义顺序不决定
`Out` 回写 ABI。
