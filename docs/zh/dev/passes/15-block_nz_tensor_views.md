...
| HOST 中不带标量索引的 `tensor.slice`（`w[0:1]`） | 拒绝——同上 |
...
可以接受；以下三种都会被拒绝：`w[r, 2:4]` 会丢掉 `2:4` 窗口；在首轴以外的轴上做标量
索引；`w[0:1]` 这样只有区间的切片，它根本没有索引可以生成。请先把整个分片传给 per-rank
函数，再在其中收窄其余轴。该限制只作用于 HOST 函数；
