...
| **NoScalarKernelReturn** | NoScalarKernelReturn | 任何 device 函数（InCore / AIC / AIV / Group / Spmd）都不得声明标量返回值——检查会递归进入 `TupleType`，因为 `-> pl.Tuple[...]` 标注只占 `return_types_` 的**一个**条目，而元组元素同样没有载体。这些函数类型的含义就是"一次可派发的任务"，而运行时的两条任务参数通道是分离的——`Arg::add_scalar` 只把标量按值传**入**，`TaskOutputTensors` 只返回 ChipTensor——因此这种返回值根本没有载体。Orchestration codegen 曾为它生成未定义标识符，后来又生成静默错误的 `= 0`（issue #631）。检查落在**声明**而非调用点：`Opaque` 父函数要到 `OutlineIncoreScopes` 才被提升为 `Orchestration`，按调用点检查会漏掉该 pass 之前写出的每一处派发。`Scalar[TASK_ID]` 豁免——它是调度句柄而非数据，由调用点从 `task_<n>_outs.task_id()` 绑定。核内的标量辅助函数应写成 `FunctionType::Inline`，由 `InlineFunctions` 在 codegen 之前展开。列入 **`GetStructuralProperties()`**，且不由任何 Pass 产生：这里不依赖任何下降结果，因此用户写出的非法签名在流水线入口即被拒绝，任何合成出该形态的 Pass 也会在自己的边界被抓住。`OutlineIncoreScopes` 通过把调用方可计算的标量外提出作用域体来维持该不变量（参见 [08-outline_incore_scopes.md](08-outline_incore_scopes.md)）。**修复方式**：把该值写入一个 `[1]` 的 tensor 输出，在派发之后用 `pl.tensor.read(t, [0])` 读回。 |
