...
| **AivSplitLoweredValid** | AivSplitLoweredValid | 共享 AIV split 验证器的 lowered 阶段契约。保留与合成区域继续遵守结构放置、跨界、归约和传输规则；标有 `split_aiv` 的 InCore 函数也可保留 flat tile 边界。仍检查结果内存。ExpandMixedKernel 按生产 lane 的可用性检查操作数，保留共享参数不受内存注解限制的兼容行为。由 LowerAutoVectorSplit 产生，由 ExpandMixedKernel 要求并失效。 |
