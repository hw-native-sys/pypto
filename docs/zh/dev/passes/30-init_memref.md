...
打印器输出的形式）。引用同一块分配的 tile 共享它，且 `MemoryReuse` 绝不会把其他 tile 塞进去。这是手工复用控制——作者为何需要它,见 [MemoryReuse](32-memory_reuse.md#声明式分配)。
...
[MemoryReuse](32-memory_reuse.md#声明式分配) 中检查。位于*不同*槽位的 tile 本来就应该同时
存活，这正是多槽位声明的用途。
