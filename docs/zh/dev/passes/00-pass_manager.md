...
tensor 前缀在 `FlattenCallExpr` 后立即以 [`AutoTile`](07-auto_tile.md) 结束。
只有函数带有 `attrs={"auto_tile": True}` 时它才工作。对于 marked function，它必须把
整个静态 tensor DAG 替换为一个 Ascend 910B Vector 调度；它不会拆图或回退为多个
kernel。
