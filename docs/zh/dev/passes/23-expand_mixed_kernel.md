...
该整函数检查只读取**单个** `func->GetSplitMode()`,无法表达多模式函数。当 `ExpandMixedKernel` 运行时,任何一等公民 `SplitAivScopeStmt` 区域均已被 [`LowerAutoVectorSplit`](22-lower_auto_vector_split.md)(pass 22)消费并擦除,后者已用**每个区域**各自的切分轴校验其转置风险,并在函数上打 `split_aiv_region_validated`。因此本 pass 对携带 `split_aiv_region_validated` 的函数跳过单一函数级模式的转置检查(AUTO 整函数路径保持不变);作用域节点永不到达此处 —— 只剩逐算子的 `aiv_shard` / `aic_gather` 标记。
...
`pl.SplitMode.NONE`——会被 [`LowerAutoVectorSplit`](22-lower_auto_vector_split.md)（pass 22）标记 `split_aiv`，因此
...
[`LowerAutoVectorSplit`](22-lower_auto_vector_split.md) 在擦除区域包装前打上（该文档说明了
...
[LowerAutoVectorSplit](22-lower_auto_vector_split.md) 对 ragged 边界做了均分时，则取算子上
