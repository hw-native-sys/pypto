...
会保留函数原有的标量填充循环。因此，常量初始化后跟动态标量覆盖的模式仍走
D-cache 路径。最终的一致性检查仍会拒绝显式混合存储。
...
本 pass 将两者**各自 1:1**下降为对应的 tile 算子（`tensor.aiv_shard` → `tile.aiv_shard`,`tensor.aic_gather` → `tile.aic_gather`）；此后 IR 与 AUTO `pl.split` 路径经 [`LowerAutoVectorSplit`](24-lower_auto_vector_split.md)（pass 24）产出的结果逐字节一致。随后 `ExpandMixedKernel`（pass 25）将两者折叠进跨核 `tpush`/`tpop` 机制。
