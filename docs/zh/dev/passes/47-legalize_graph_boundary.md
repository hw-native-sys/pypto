...
| 循环内的 create | 它是**每次迭代一份**的新 buffer。把 N 份塌缩成一个形参会让各迭代互相别名，而本该重新串行化它们的跨任务依赖边，是更早的 [`AutoDeriveTaskDependencies`](41-auto_derive_task_dependencies.md) 推导出来的 |
...
跑在最后一个 `Simplify` 之后、
[`MaterializeRuntimeScopes`](48-materialize_runtime_scopes.md) 之前。
...
- [MaterializeRuntimeScopes](48-materialize_runtime_scopes.md) —— 紧随其后运行
