...
| 51 | [InsertCommFence](51-insert_comm_fence.md) | 阶段 A：在 InCore 体内为发布写和 wait 打标记；阶段 B：当函数体对 `DistributedTensorType` 值做标量读（`tensor.read`）时，在 InCore 入口 prepend 仅 invalidate 的 whole-GM `system.cacheinvalid`；notify 本身不加标记 |
