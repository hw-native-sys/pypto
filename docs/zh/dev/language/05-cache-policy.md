...
> **要求 PTOAS >= v0.61**，也就是 `toolchain/versions.env` 所固定的版本。`BYPASS`
...
`toolchain/versions.env` 固定 PTOAS >= v0.61，因此该发射是无条件的 —— 没有版本门控，
代码树中也没有任何机制在编译期读取所固定的汇编器版本。指向更旧汇编器的构建因此处于契约
之外：`cache_policy` 是 v0.61 新增的，预期会在那里无法通过 `pto.tload` 的校验。
