...
| pto.tfillpad | TFILLPAD / PyPTO fillpad-expand | tile+tensor | ✅ | ✅ | ✅ | ✅ | — | PTOAS v0.58 没有 `pto.tfillpad_expand`；扩展型 `pto.tfillpad` 会生成 `TFILLPAD<pto::TFillPadMode::Expand>`，PyPTO 在 C++ 预处理阶段将其适配为固定 PTOISA 中等价的 `TFILLPAD_EXPAND` 调用 |
