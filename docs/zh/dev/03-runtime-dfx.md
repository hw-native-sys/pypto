...
kernel-mode JIT 调用通过 `pypto.torch.init` 配置 DFX，使用
`pypto.torch.begin_dfx()` / `end_dfx()` 包围待测调用，详见
[kernel 泳道图采集](runtime/kernel-mode.md#kernel-swimlane-collection)。
下文的 `RunConfig` 选项适用于 program 执行。
