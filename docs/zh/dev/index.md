...
PR 运行。同一个 PR 的准备任务会串行执行，最多保留 100 个等待中的准备任务；
队列已满时，新到达的任务会被取消。
...
避免静默保留旧的 PR 检查状态。只读查询使用具有 **Actions: read** 和 **Checks: read** 权限的
`GITHUB_TOKEN`。

路由从 PR 编号出发，完整分页读取当前提交的 GraphQL `statusCheckRollup`，
通过 Actions 检查获取工作流运行的 `resourcePath`，严格核对仓库路径后提取运行 ID，
避免依赖 GraphQL 整数宽度。每一页都必须仍对应请求审查的 PR head。
随后核对工作流、事件、目标仓库，以及产物中的仓库、PR 编号、head SHA 和
目标分支，不依赖工作流运行的源分支元数据。head 发生变化时会停止路由，
也不会回退到独立审查。小型身份产物保留
