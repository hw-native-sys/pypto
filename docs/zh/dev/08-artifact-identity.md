...
一个组件可以**同时**带版本与内容。设备工具链就是如此：它的编译器跑在宿主上，因此
输入来自两个来源——CANN 安装（会自报构建）与宿主 OS 提供的文件（什么都不报）。
每一部分由它**实际拥有**的证据覆盖。声明出来的 roots 始终会被读取，因此版本永远
不会替代组件自己列出的文件；若那份清单读不出来，无论版本多好，该组件都不可用。

CANN 由 `ascend_toolkit_install.info` 里的 `innerversion` 标识，优先于 `version`：
同一发布的两次构建共享发布号，只有构建号不同。安装会通过不止一条路径抵达自己的
info 文件——`arm64-linux` 是指向 `aarch64-linux` 的符号链接——因此候选项按**它们
指向的文件**计数，而不是按路径计数。任何意外情况（没有 info 文件、有两个不同的、
值为空、安装布局不认识）都退回到与此前一样读取全部输入。
