...
对 `Tensor` 和 `Tile` 输入，`pl.cast` 还接受仅关键字参数 `saturation_mode`（`"on"` / `"off"`，或 `1` / `0`）。`"on"` 把超出目标值域的结果钳制到该值域；`"off"` 保留目标平台的非饱和转换，其溢出行为由架构定义。**默认值是 `"on"`** —— 在两者之间，"意外得到钳制"更安全，而且在 A2/A3 上它正是硬件原生支持的转换。当回绕才是算子约定时，显式传入 `"off"`。多跳 cast 只把该模式作用于最后一跳，即真正到达你所指定 dtype 的那一跳。
