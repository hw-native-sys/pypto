...
> **致命陷阱：**
>
> - 一前一后写两次派发并不会让它们有序。如果那个关系没有体现为运行时能看见的缓冲区重叠，就不存在边，两个任务可能重叠执行。结果是一个偶发复现、一上调试器就消失的竞态。请用 `deps=` 把边写出来。
> - 对一个生产者不在 `deps=` 里的张量使用 `predicate=`，读到的是当时恰好在那里的东西。没有任何提示 —— 任务被跳过或不被跳过，取决于陈旧数据。
