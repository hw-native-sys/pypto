...
块——于是无论 `P` 多大，每步传输都保持 `N/P`。同步也是邻居局部的：每个
...
总字节数与两阶段相同（`2 * (P-1) / P * N`），但每步只移动 `N/P`——随着 `P`
增长每步大小恒定，这正是 ring 在大 world 规模下依然高效的原因。
...
- **邻居就绪握手，而非 barrier。** 每轮有自己的信号行——store 之后通知**右**
  ...
  参与同步——总计 O(P) 次信号，而不是每轮全 barrier 的 O(P²)。

**成本卡（每 rank）：** 总计 `2 * (P-1) / P * N`——与两阶段相同——但分在
`2*(P-1)` 步、每步 `N/P` 字节。每步大小**随 P 增长而恒定**，不像 mesh 每步
`(P-1) * N`；这正是 ring 存在的原因。

## 边界情况（Edge cases）

> **致命陷阱——左邻居索引取负。** `(my_rank - 1) % nranks` 在 rank 0 处截断
> 取模得 `-1`：`remote_load` 会指向一个无效对端，ring 在 P=2 死锁（这是
> rank 0 的左邻居恰好是自己的唯一情形）。**修复：** 始终写
