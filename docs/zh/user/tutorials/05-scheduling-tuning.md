...
每一步都是一个写出文件的 `RunConfig` 开关。这里没有任何一步需要你去改造 kernel。
...
| 任务真的重叠了吗？ | `enable_l2_swimlane=True` | `<work_dir>/dfx_outputs/l2_swimlane_records.json` |
...
> **模拟器注意事项：** 在 `*sim` 平台上这是单趟的，只产出 `l2_swimlane_records.json`。合并后的 `merged_swimlane_*.json` 视图会被**有意跳过**，因为模拟器尚未提供转换器所需的任务元数据。在真机平台上同一个开关会把负载**跑两遍** —— 先一趟 dep_gen 采集图，再一趟干净的计时 —— 因为采集会扰动时序。
