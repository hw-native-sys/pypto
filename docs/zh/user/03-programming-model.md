...
一个 program 被划分到两个面上，每个函数恰好属于其一：
...
`FunctionType` 记录一个函数属于哪个面：

| 取值 | 所属面 | 含义 |
| ---- | ------ | ---- |
| `Opaque` | —— | 默认。无特定执行上下文；可作为构件使用 |
| `Orchestration` | 控制面 | 主机侧编排者 —— 分配张量、派发 kernel |
| `InCore` | 执行面 | AICore 上的计算 kernel |
| `Inline` | —— | 由第一个 pass 展开到每个调用点；最终不留下函数 |
...
对写代码的直接影响是：**编排函数里语句的先后顺序不等于执行顺序。** 编译器依据缓冲区重叠推导
依赖，运行时会把没有先后约束的任务并发跑起来。两次访问不相交缓冲区的派发，即使一前一后写在
源码里，也可能重叠执行。需要特定顺序的地方，要用依赖去表达，而不是靠语句位置。
...
> **致命陷阱：** Orchestration 函数里的语句顺序不约束执行顺序。如果两次派发必须有先后、却访问
> 不同的缓冲区，源码里没有任何东西表达了这一点 —— 运行时可以自由地让它们重叠，结果是一个偶发
> 复现、一上调试器就消失的竞争。

| Symptom | Likely Cause | Fix |
| ------- | ------------ | --- |
| **多次运行结果不一致** | 两个任务之间没有依赖相连，发生竞争 | 显式建立依赖，不要依赖语句顺序 |
