...
| `assemble` | `(target: Tensor, source: Tensor, offset: Sequence[IntLike], *, atomic: AtomicType = AtomicType.None_) -> Tensor` | 将 source 写入 target 的指定偏移。语法糖（仅 SSA 前）：`target[i:i+H, j:j+W] = source`。`atomic=AtomicType.Add` 改为累加而非覆盖（split-K）——仅当写在 **InCore 函数内部**（通常即 `pl.at(level=pl.Level.CORE_GROUP, ...)` 作用域）且 target 为函数输出（全局内存）时合法；写在作用域外部（编排层）的原子 assemble 会在编译期报错，因为编排层没有任何可执行原子累加的指令。浮点结果不确定，target 需预先清零，支持 dtype fp32/bf16/fp16/int32/int16/int8（bf16 仅在 Ascend910B/A2/A3 上支持） |
