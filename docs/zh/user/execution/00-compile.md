...
| `output_dir` | `None` | 产物落点；`None` 表示每次调用在 `PYPTO_PROG_BUILD_DIR` 或 `build_output` 下新建一个 `<name>_<YYYYmmdd_HHMMSS>_<random>`。时间戳便于阅读，随机尾缀保证并发编译互不覆盖，因此路径不可推导 —— 请读 `compiled.output_dir` |
