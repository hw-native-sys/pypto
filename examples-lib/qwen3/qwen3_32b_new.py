...
def build_qwen3_single_layer_decode_program(
    ...
    HALF_DIM = HEAD_DIM_CFG // 2
    Q_GROUPS_PER_KV = Q_PER_KV_CFG // Q_HEAD_BATCH
    TOTAL_Q_GROUPS = NUM_KV_HEADS_CFG * Q_GROUPS_PER_KV
    ...
    class Qwen3SingleLayerDecode:
        ...
        def qwen3_decode_layer(
            ...
            # Split into separate pl.incore() blocks so each outlined kernel
            # has a single cross-core payload size (avoids MixedKernelExpanded
            # slot-size conflicts). Structure follows decode_scope2 pattern.
            for b in pl.parallel(BATCH_CFG):
                ...
                # Stage 1b: apply RoPE to K halves.
                k_rot_tensor = pl.create_tensor([NUM_KV_HEADS_CFG, HEAD_DIM_CFG], dtype=pl.FP32)
                with pl.incore():
                    k_lo = pl.slice(k_group, [NUM_KV_HEADS_CFG, HALF_DIM], [0, 0])
                    k_hi = pl.slice(k_group, [NUM_KV_HEADS_CFG, HALF_DIM], [0, HALF_DIM])
                    rot_lo = pl.sub(
                ...
                # Stage 1c: write rotated K and V to cache.
                with pl.incore():
                    for ki in pl.range(NUM_KV_HEADS_CFG):
                ...
                for gi in pl.parallel(0, TOTAL_Q_GROUPS, 1):
                    ...
                    with pl.incore():
                        ...
                        q_rot_bf16 = pl.assemble(
                            q_rot_bf16, pl.cast(q_rot_lo, target_type=pl.BF16), [0, 0])
                        q_rot_bf16 = pl.assemble(
                            q_rot_bf16, pl.cast(q_rot_hi, target_type=pl.BF16), [0, HALF_DIM])
                    ...
                    for sb in pl.range(ctx_blocks):
                        ...
                        with pl.incore():
                            k_tile = pl.slice(
                                k_cache, [SEQ_TILE, HEAD_DIM_CFG], [cache_row0, 0])
                            raw_scores_pad = pl.matmul(
                                q_padded, k_tile, b_trans=True, out_dtype=pl.FP32)

                        # Softmax (pure Vector).
                        exp_scores_bf16 = pl.create_tensor([Q_HEAD_BATCH, SEQ_TILE], dtype=pl.BF16)
                        with pl.incore():
                            ...
                            scores_scaled = pl.mul(scores_padded, ATTN_SCALE)
                            cur_mi = pl.row_max(scores_scaled)
                            exp_scores = pl.exp(pl.row_expand_sub(scores_scaled, cur_mi))
                            exp_scores_bf16_t = pl.cast(exp_scores, target_type=pl.BF16)
                            exp_scores_fp32 = pl.cast(exp_scores_bf16_t, target_type=pl.FP32)
                            cur_li = pl.row_sum(exp_scores_fp32)
                            exp_scores_bf16 = pl.assemble(
                                exp_scores_bf16, exp_scores_bf16_t, [0, 0])

                        # Pad exp_scores for SV matmul.
                        exp_padded = pl.create_tensor([Q_HEAD_PAD, SEQ_TILE], dtype=pl.BF16)
                        with pl.incore():
                            exp_tile = pl.slice(
                                exp_scores_bf16, [Q_HEAD_BATCH, SEQ_TILE], [0, 0])
                        ...
                        with pl.incore():
                            v_tile = pl.slice(
                                v_cache, [SEQ_TILE, HEAD_DIM_CFG], [cache_row0, 0])
                            oi_tmp_pad = pl.matmul(
                                exp_padded, v_tile, out_dtype=pl.FP32)

                        # Online softmax update (pure Vector).
                        with pl.incore():
                            oi_tmp = pl.slice(
                                oi_tmp_pad, [Q_HEAD_BATCH, HEAD_DIM_CFG], [0, 0])
                            if sb == 0:
                                oi = oi_tmp
                                li = cur_li
                                mi = cur_mi
                    ...
                    # Normalize and write to attn_row.
                    with pl.incore():
...
def build_tensor_specs(
    ...
    cache_pos_data = torch.randint(0, max_seq_len, (batch,), dtype=torch.int32)

    return [
        TensorSpec("hidden_states", [batch, hidden_size], torch.bfloat16, init_value=torch.randn),
        TensorSpec("cache_pos", [batch], torch.int32, init_value=cache_pos_data),
