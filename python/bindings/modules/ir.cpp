...
#undef BIND_UNARY_EXPR
  ...
  ir.def(
      ...
      nb::arg("stmt"),
      "Unwrap a statement into a flat list. Returns children of SeqStmts/OpStmts, or a single-element list.");

  ir.def(
      "wrap_in_seq_stmts",
      [](const std::vector<StmtPtr>& stmts, const Span& span) {
        return transform_utils::WrapInSeqStmts(stmts, span);
      },
      nb::arg("stmts"), nb::arg("span"), "Wrap a list of statements into a single SeqStmts node.");
