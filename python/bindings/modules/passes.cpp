...
void BindPass(nb::module_& m) {
  ...
  nb::enum_<IRProperty>(passes, "IRProperty", "Verifiable IR properties")
      ...
      .value("DeferredCompositePlacementValid", IRProperty::DeferredCompositePlacementValid,
             "Every pld.tensor.*(defer=True) Call sits in a task-level pl.at(CORE_GROUP) body "
             "without allow_early_resolve or a dispatch predicate (deferred-placement check)");
  ...
  passes.def("split_deferred_composite_kernels", &pass::SplitDeferredCompositeKernels,
             "Split InCore kernels that mix payload with pld.system.defer_wait into push/wait/epi\n"
             "functions and rewrite Orchestration Submit sites. Runs immediately before\n"
             "ExpandMixedKernel so deferred_completion_waiter bodies stay registration-only.");
