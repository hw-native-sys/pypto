...
class TypeResolver:
    ...
    def _get_type_name(self, node: ast.expr) -> str | None:
        """Extract the type name from an AST node referencing a known IR type.
        ...
        if isinstance(node, ast.Attribute) and node.attr in (
            ...
            return node.attr
        if isinstance(node, ast.Name) and node.id in ("Tensor", "Tile", "Scalar", "Tuple", "UnknownType"):
            return node.id
        return None
    ...
    def resolve_type(self, type_node: ast.expr) -> "ir.Type | list[ir.Type]":
        ...
        if type_name == "UnknownType":
            return ir.UnknownType.get()
    ...
    def _validate_shape_value(self, value: Any, source_name: str, span: ir.Span) -> list[int | ir.Expr]:
        ...
        for i, elem in enumerate(value):
            if isinstance(elem, int):
                ...
                elem._ir_var = self._get_or_create_dyn_var(elem, elem.name, span)
    ...
    def _validate_dim_value(self, value: Any, source_name: str, span: ir.Span) -> int | ir.Expr:
        ...
        if isinstance(value, DynVar):
            value._ir_var = self._get_or_create_dyn_var(value, value.name, span)
    ...
    def _parse_tileview_expr(self, node: ast.expr) -> "ir.Expr":
        ...
        if isinstance(node, ast.Name):
            ...
            if name in self.expr_evaluator.closure_vars:
                ...
                if isinstance(value, DynVar):
                    value._ir_var = self._get_or_create_dyn_var(value, value.name, self._get_span(node))
                    return value._ir_var
                if isinstance(value, int):
