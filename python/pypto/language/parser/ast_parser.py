...
class ASTParser:
    ...
    def _parse_at_predicate(
        ...
        """Parse ``pl.at(..., predicate=(t[i] > 0))`` — CORE_GROUP, top-level only.

        A dispatch predicate only has a runtime carrier where the scope becomes
        an independently submitted L0 task, i.e. an InCore scope outlined by
        ``OutlineIncoreScopes`` into a ``Submit`` that orchestration codegen
        dispatches directly. Two placements would silently drop it instead:

        * a non-CORE_GROUP ``pl.at`` builds a Hierarchy scope, which
          ``OutlineHierarchyScopes`` only rewrites inside ``Opaque`` functions
          and which never lowers to a ``Submit`` in an Orchestration function;
        * a ``pl.at`` nested inside ``pl.cluster()`` / ``pl.spmd()`` / another
          ``pl.at`` is folded into the enclosing Group / Spmd wrapper dispatch
          (or into the enclosing kernel body), and codegen emits
          ``set_predicate`` from the *outer* call only.

        Both are rejected here rather than miscompiled, mirroring
