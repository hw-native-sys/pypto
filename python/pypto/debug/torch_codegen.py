...
def _handle_tensor_a8w8_matmul_dequant_acc(a: list[str], kw: dict[str, Any]) -> str:
    acc, lhs, rhs, act_scale, weight_scale = a[0], a[1], a[2], a[3], a[4]
    if kw.get("a_trans"):
        lhs = f"{lhs}.mT"
    if kw.get("b_trans"):
        rhs = f"{rhs}.mT"
    expr = (
        f"(({acc}.to(torch.float32) + torch.matmul({lhs}.to(torch.float32), {rhs}.to(torch.float32))) "
        f"* {act_scale}.to(torch.float32) * {weight_scale}.to(torch.float32))"
    )
    out_dtype = kw.get("out_dtype")
    if isinstance(out_dtype, DataType):
...
def _register_ops() -> None:  # noqa: PLR0915
    ...
    m["tensor.a8w8_matmul_dequant_acc"] = _handle_tensor_a8w8_matmul_dequant_acc
