...
| 20 | `20-fold_fixpipe_acc_epilogue.md` | Folds a vector dequant/ReLU epilogue (`cast`/`muls`/`maximums`) into `pre_quant`/`pre_relu` on the cube's Acc->GM `tile.store`, so a scaled matmul stays a pure-cube kernel instead of splitting into AIC+AIV with a C2V round-trip. Runs immediately before `InferTileMemorySpace`, which is what would otherwise pin the chain to `Mem.Vec` |
...
| 54 | `54-lower_tile_to_buffer.md` | Lowers tile ops onto the Buffer IR; runs after MaterializeValidShapeSymbols |
