...
/// Find the first YieldStmt inside a statement body (searches through SeqStmts/OpStmts).
...
/// this function only looks at the back of SeqStmts/OpStmts containers, finding
...
inline YieldStmtPtr GetLastYieldStmt(const StmtPtr& body) {
  ...
  if (auto ops = As<OpStmts>(body)) {
    ...
    return GetLastYieldStmt(ops->stmts_.back());
  }
...
///
/// If the statement is a SeqStmts or OpStmts, returns its children;
...
/// Wrap a vector of statements into a single SeqStmts node.
inline StmtPtr WrapInSeqStmts(const std::vector<StmtPtr>& stmts, const Span& span) {
