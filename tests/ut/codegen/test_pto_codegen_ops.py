...
class TestColReductionCodegen:
    ...
    def test_col_sum_codegen(self):
        """tile.col_sum default emits pto.tcolsum with isBinary = false."""
        ...
        assert "isBinary = false" in mlir, f"Expected isBinary = false (default) in codegen output:\n{mlir}"

    def test_col_sum_codegen_binary(self):
        """tile.col_sum with is_binary=True emits isBinary = true."""
        ...
        class Prog:
            ...
            def main(
                ...
                result: pl.Tile[[1, 16], pl.FP32] = pl.tile.col_sum(tile_in, tmp_tile, is_binary=True)
                return pl.store(result, [0, 0], output)
