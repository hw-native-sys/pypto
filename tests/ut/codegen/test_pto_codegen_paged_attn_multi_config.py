            ...
            for b_idx in pl.range(batch):
                for q_idx in pl.range(q_loop_static):
                    ...
                    for bn in pl.range(0, max_bn, n_unroll):
                        ...
                        if bn == 0:
                            is_first: pl.Scalar[pl.INDEX] = pl.yield_(pl.const(1, pl.INDEX))
                        else:
                            is_first: pl.Scalar[pl.INDEX] = pl.yield_(pl.const(0, pl.INDEX))
                        if bn + n_blocks == max_bn:
                            is_last: pl.Scalar[pl.INDEX] = pl.yield_(pl.const(1, pl.INDEX))
                        else:
                            is_last: pl.Scalar[pl.INDEX] = pl.yield_(pl.const(0, pl.INDEX))
