...
def test_external_mixed_kernel_codegen(tmp_path):
    ...
    # 1. The hand-written source is copied verbatim into both core paths (one
    #    func_id per core), not regenerated by ptoas.
    aic_cpp = out_dir / "kernels" / "aic" / "K_AIC.cpp"
    aiv_cpp = out_dir / "kernels" / "aiv" / "K_AIV.cpp"
    assert aic_cpp.read_text() == _KERNEL_SRC
    assert aiv_cpp.read_text() == _KERNEL_SRC

    # 2. The manifest lists both kernels with consecutive func_ids, the right
    #    core types, and sources pointing at the copied files.
    ...
    assert '"kernels" / "aic" / "K_AIC.cpp"' in config
    assert '"kernels" / "aiv" / "K_AIV.cpp"' in config
