...
These run the full Default pass pipeline via ``lower`` (no device). They do
NOT exercise the per-pass print -> reparse roundtrip: ``tests/ut/jit/conftest.py``
replaces the ``tests/ut/conftest.py`` verification fixture with a no-op, so the
dump marks' print fidelity is covered by the parser tests
(``tests/ut/language/parser/test_dump_tag_dsl.py`` / ``test_dump_dsl.py``) and
the pass tests instead. The companion device/manifest checks live in
``tests/st/runtime/framework_and_models/test_dump_tag.py``.
"""
