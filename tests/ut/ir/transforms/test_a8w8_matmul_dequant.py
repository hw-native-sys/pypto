...
def test_a8w8_matmul_dequant_acc_lowers_to_accumulate_then_dequant():
    """The acc fusion op lowers to INT8 matmul_acc followed by vector dequant."""
    ...
    class Program:
        ...
        def kernel(
            self,
            acc_i32: pl.Tensor[[16, 256], pl.INT32],
            lhs_i8: pl.Tensor[[16, 512], pl.INT8],
            rhs_i8: pl.Tensor[[512, 256], pl.INT8],
            act_scale: pl.Tensor[[16, 1], pl.FP32],
            weight_scale: pl.Tensor[[1, 256], pl.FP32],
        ) -> pl.Tensor[[16, 256], pl.BF16]:
            out = pl.a8w8_matmul_dequant_acc(
                acc_i32, lhs_i8, rhs_i8, act_scale, weight_scale, out_dtype=pl.BF16
            )
            return out
    ...
    assert "pl.tile.matmul_acc(" in text
    assert "pl.tile.cast(" in text
    assert "target_type=pl.FP32" in text
    assert "pl.tile.cast(" in text
    assert "pl.tile.row_expand_mul(" in text
...
def test_a8w8_matmul_dequant_acc_decode_m1_uses_gemv_acc():
    """Decode M=1 uses gemv_acc and scalar activation scale."""
    ...
    class Program:
        ...
        def kernel(
            self,
            acc_i32: pl.Tensor[[1, 256], pl.INT32],
            lhs_i8: pl.Tensor[[1, 512], pl.INT8],
            rhs_i8: pl.Tensor[[512, 256], pl.INT8],
            act_scale: pl.Tensor[[1, 1], pl.FP32],
            weight_scale: pl.Tensor[[1, 256], pl.FP32],
    ...
    assert "pl.tile.gemv_acc(" in text
    assert "pl.tile.matmul_acc(" not in text
    assert "pl.tile.read(" in text
    assert "pl.tile.muls(" in text
    assert "pl.tile.mul(" in text
    assert "pl.tile.row_expand_mul(" not in text
