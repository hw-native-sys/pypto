...
def make_sync_id_var(index: int) -> ir.Var:
    """Create Var('sync_id_N', ScalarType(BOOL))."""
    return ir.Var(f"sync_id_{index}", ir.ScalarType(DataType.BOOL), _span)


def make_sync_id_init(sync_id_var: ir.Var) -> ir.AssignStmt:
    """Create AssignStmt: sync_id = false (0)."""
    return ir.AssignStmt(sync_id_var, ir.ConstInt(0, DataType.BOOL, _span), _span)


def make_sync_id_set(sync_id_var: ir.Var) -> ir.AssignStmt:
    """Create AssignStmt: sync_id = true (1)."""
    return ir.AssignStmt(sync_id_var, ir.ConstInt(1, DataType.BOOL, _span), _span)


def make_conditional_wait(
    sync_id_var: ir.Var, set_pipe: ir.PipeType, wait_pipe: ir.PipeType, event_id: int
) -> ir.IfStmt:
    """Create IfStmt: if(sync_id) { sync_dst(...); sync_id = false }."""
    then_body = ir.SeqStmts(
        [ir.OpStmts([make_sync_dst(set_pipe, wait_pipe, event_id), make_sync_id_init(sync_id_var)], _span)],
        _span,
    )
    return ir.IfStmt(sync_id_var, then_body, None, [], _span)
...
def test_insert_sync_cube_pipe():
    ...
    move_a = block.move(tile_a, target_memory=ir.MemorySpace.L0A.value)
    move_b = block.move(tile_b, target_memory=ir.MemorySpace.L0B.value)
        ...
        [
            ir.OpStmts(
                [
                    ...
                    ir.AssignStmt(
                        tile_a_cube, block.move(tile_a, target_memory=ir.MemorySpace.L0A.value), span
                    ),
                    make_sync_dst(MTE2, MTE1, 1),
                    ir.AssignStmt(
                        tile_b_cube, block.move(tile_b, target_memory=ir.MemorySpace.L0B.value), span
                    ),
    ...
    expected_else = ir.SeqStmts(
        [
            ir.OpStmts(
                [
                    make_sync_dst(MTE2, V, 0),
                    ir.AssignStmt(tile_c, block.mul(tile_a, tile_a), span),
                ],
                span,
            ),
            ir.YieldStmt([tile_c], span),
        ...
        [
            ir.OpStmts(
                [
                    ...
                    ir.AssignStmt(tile_a, block.load(input_tensor, offsets=[0, 0], shapes=[64, 64]), span),
                    make_sync_src(MTE2, V, 0),
                    make_sync_id_set(sid0),
                ],
                span,
            ),
            ir.IfStmt(condition, expected_then, expected_else, [], span),
            make_conditional_wait(sid0, MTE2, V, 0),
            ir.ReturnStmt(span),
        ],
        span,
    )
...
def test_for_loop():
    ...
    expected_body = ir.SeqStmts(
        [
            ...
            make_conditional_wait(sid1, V, MTE3, 0),
            ir.OpStmts(
                [
                    ir.AssignStmt(
                        store_result,
...
def test_for_cross_iteration():
    ...
    expected_for_body = ir.SeqStmts(
        [
            make_conditional_wait(sid0, V, MTE2, 0),
            ir.OpStmts(
                [
                    ir.AssignStmt(tile_a, block.load(input_tensor, offsets=[0, 0], shapes=[64, 64]), span),
                    make_sync_src(MTE2, V, 0),
    ...
    expected_body = ir.SeqStmts(
        [
            ir.OpStmts([make_sync_id_init(sid0)], span),
            ir.ForStmt(loop_var, start, stop, step, [], expected_for_body, [], span),
            make_conditional_wait(sid0, V, MTE2, 0),
            ir.ReturnStmt(span),
        ],
...
def test_for_cross_iteration_mte3_to_mte2():
    ...
    expected_for_body = ir.SeqStmts(
        [
            make_conditional_wait(sid0, MTE3, MTE2, 0),
            ir.OpStmts(
                [
                    ir.AssignStmt(tile_a, block.load(data_tensor, offsets=[0, 0], shapes=[64, 64]), span),
                    make_sync_src(MTE2, V, 0),
