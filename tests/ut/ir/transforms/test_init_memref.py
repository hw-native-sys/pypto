...
def test_init_memref_matmul():
    ...
    with ib.function("test_load_move_matmul") as f:
        # Define input and output parameters (Global Tensors -> DDR)
        ...
        # Load tile_a to UB (target_space=0, default)
        tile_a_ub = ib.let("tile_a_ub", block.load(input_a, 0, 0, tile_height, tile_width, target_space=0))

        # Load tile_b to L1 (target_space=1)
        tile_b_l1 = ib.let("tile_b_l1", block.load(input_b, 0, 0, tile_height, tile_width, target_space=1))

        # Move tile_a from UB to L0A (target_space=2)
        tile_a_l0a = ib.let("tile_a_l0a", block.move(tile_a_ub, target_space=2))

        # Move tile_b from L1 to L0B (target_space=3)
        tile_b_l0b = ib.let("tile_b_l0b", block.move(tile_b_l1, target_space=3))
