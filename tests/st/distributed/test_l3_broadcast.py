...
def _make_rank_inputs(n_ranks: int) -> torch.Tensor:
    """Rank 0 is root with distinct values; non-root values must not leak."""
    rows = [
        torch.arange(SIZE, dtype=torch.float32).reshape(1, SIZE),
    ]
    for r in range(1, n_ranks):
        rows.append(
            torch.arange((r + 1) * 100.0, (r + 1) * 100.0 + SIZE, dtype=torch.float32).reshape(1, SIZE)
        )
...
class BroadcastMesh:
    ...
    def broadcast_step(
        ...
        # Phase 1: stage-in — root only writes its input into the HCCL window.
        if my_rank == root:
        ...
        # Phase 3: broadcast — read root scratch, write local output.
...
class TestL3Broadcast:
    ...
    def test_broadcast(self, test_config, device_ids, n_ranks):
        ...
        # Non-root inputs must not leak into any output.
        for r in range(1, n_ranks):
