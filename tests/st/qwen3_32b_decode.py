            ...
            for b in pl.parallel(BATCH_CFG):
                ...
                with pl.incore():
                    ...
                    for ki in pl.range(NUM_KV_HEADS_CFG):
                        ...
                        k_group = pl.assemble(
                            k_group,
                            pl.cast(pl.slice(k_proj, [1, HEAD_DIM_CFG], [b, kv_col]), target_type=pl.FP32),
                            [ki, 0],
                        )
                ...
                with pl.incore():
                    ...
                    k_hi = pl.slice(k_group, [NUM_KV_HEADS_CFG, HEAD_DIM_CFG // 2], [0, HEAD_DIM_CFG // 2])
                    k_rot = pl.concat(
                    ...
                    for ki in pl.range(NUM_KV_HEADS_CFG):
                        ...
                        k_cache = pl.assemble(
                            k_cache,
                            pl.cast(pl.slice(k_rot, [1, HEAD_DIM_CFG], [ki, 0]), target_type=pl.BF16),
                            [cache_row, 0],
                        )
                        v_cache = pl.assemble(
                            v_cache,
                ...
                for gi in pl.parallel(0, TOTAL_Q_GROUPS, 1):
                    ...
                    with pl.incore():
                        ...
                        for qi in pl.range(Q_HEAD_BATCH):
                            ...
                            q_group = pl.assemble(
                                q_group,
                                pl.cast(pl.slice(q_proj, [1, HEAD_DIM_CFG], [b, q_col]), target_type=pl.FP32),
                                [qi, 0],
                            )
                    ...
                    with pl.incore():
                        ...
                        q_hi = pl.slice(q_group, [Q_HEAD_BATCH, HEAD_DIM_CFG // 2], [0, HEAD_DIM_CFG // 2])
                        q_rot = pl.concat(
