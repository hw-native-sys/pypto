...
def _values(dtype: DataType) -> torch.Tensor:
    """Build deterministic zero, sign-bit, and extremal source bit patterns."""
    width = _WIDTH[dtype]
...
def _shift_counts(dtype: DataType) -> torch.Tensor:
    """Repeat zero, one, and the maximum legal count across the input tile."""
    width = _WIDTH[dtype]
    values = [0, 1, width - 1]
    index = torch.arange(M * N, dtype=torch.int64).reshape(M, N).remainder(len(values))
    result = torch.zeros((M, N), dtype=torch.int64)
...
def _make_program(op_name: str, dtype: DataType, scalar: int | None, valid_shape: tuple[int, int]):
    """Build a JIT entry that executes exactly the requested shift instruction."""
    ...
    if op_name == "shl":
        ...
        def shl_kernel(
            ...
            """Left-shift the valid region using a same-dtype count tile."""
            src_tile = pl.load(src, [0, 0], [M, N], valid_shape=[valid_rows, valid_cols])
            shift_tile = pl.load(shift, [0, 0], [M, N], valid_shape=[valid_rows, valid_cols])
        ...
        def shl_program(
            ...
            """Expose the tile left-shift kernel through the runtime JIT entry."""
            return shl_kernel(src, shift, out)
    ...
    if op_name == "shr":
        ...
        def shr_kernel(
            ...
            """Right-shift the valid region using a same-dtype count tile."""
            src_tile = pl.load(src, [0, 0], [M, N], valid_shape=[valid_rows, valid_cols])
            shift_tile = pl.load(shift, [0, 0], [M, N], valid_shape=[valid_rows, valid_cols])
        ...
        def shr_program(
            ...
            """Expose the tile right-shift kernel through the runtime JIT entry."""
            return shr_kernel(src, shift, out)
    ...
    if op_name == "shls":
        ...
        def shls_kernel(
            ...
            """Left-shift the valid region using the scalar count."""
            src_tile = pl.load(src, [0, 0], [M, N], valid_shape=[valid_rows, valid_cols])
        ...
        def shls_program(
            ...
            """Expose the scalar left-shift kernel through the runtime JIT entry."""
            return shls_kernel(src, out)
    ...
    def shrs_kernel(
        ...
        """Right-shift the valid region using the scalar count."""
        src_tile = pl.load(src, [0, 0], [M, N], valid_shape=[valid_rows, valid_cols])
    ...
    def shrs_program(
        ...
        """Expose the scalar right-shift kernel through the runtime JIT entry."""
        return shrs_kernel(src, out)
...
class ShiftCase(PTOTestCase):
    ...
    def __init__(
        ...
        *,
        ...
        """Record the operator, dtype, valid region, and optional scalar count."""
        super().__init__()
    ...
    def get_name(self) -> str:
        """Give each dtype, shape, and count combination a distinct case name."""
    ...
    def define_tensors(self) -> list[TensorSpec]:
        """Create deterministic inputs and a zero-initialized output buffer."""
    ...
    def get_program(self) -> Any:
        """Specialize complete annotations without inferring unsigned Torch dtypes."""
        return _make_program(self.op_name, self.dtype, self.scalar, self.valid_shape).specialize()

    def compute_expected(self, tensors: dict[str, torch.Tensor], params=None) -> None:
        """Compute fixed-width shifts and leave unstored output elements at zero."""
        rows, cols = self.valid_shape
...
class TestShiftTileFamily:
    ...
    def test_shift(self, test_runner, op_name, dtype, valid_name, valid_shape):
        """Compare a tile-count shift with the fixed-width reference."""
...
class TestShiftScalarA5Family:
    ...
    def test_shift(self, test_runner, op_name, dtype, valid_name, valid_shape, scalar):
        """Compare every supported A5 scalar width and valid region."""
        result = test_runner.run(
            ShiftCase(
                op_name=op_name,
                dtype=dtype,
...
class TestShiftScalarA2A3Family:
    ...
    def test_shift(self, test_runner, op_name, dtype, valid_name, valid_shape, scalar):
        """Compare scalar shifts within the pinned A2/A3 executable shape subset."""
        result = test_runner.run(
            ShiftCase(
                op_name=op_name,
                dtype=dtype,
