...
correctly through the full Default pipeline in codegen-only mode.
...
This test intentionally stays focused on codegen-only coverage in CI.
...
class TestCrossCore:
    ...
    @pytest.mark.skip(reason="Simpler WIP")
    def test_tpush_tpop_v2c_updown(self, test_runner):
        """V2C updown pipe: compile through full pipeline and verify kernel artifacts.

        Uses --codegen-only mode by default for CI. On device, verifies
        that output = (a + b) @ (a - b) matches golden reference.
        """
