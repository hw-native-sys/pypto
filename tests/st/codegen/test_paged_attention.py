...
class PagedAttentionMultitierExampleTestCase(PagedAttentionTestCase):
    ...
    Uses a shuffled (non-contiguous) block_table to verify that the x2 tier
    correctly handles non-contiguous physical blocks by reading each block
    independently. Overrides compute_expected to mirror the actual 2-block
    kernel pipeline:
    ...
    def define_tensors(self) -> list[TensorSpec]:
        ...
        # to verify non-contiguous physical block handling.
...
class QKMatmul2BlockTestCase(PTOTestCase):
    ...
    Computes: sij = [qi @ kj0.T | qi @ kj1.T]  -> (num_heads, 2*block_size)
    """
    ...
    def define_tensors(self) -> list[TensorSpec]:
        return [
            ...
            TensorSpec("kj0", [self.block_size, self.head_dim], DataType.BF16, init_value=torch.randn),
            TensorSpec("kj1", [self.block_size, self.head_dim], DataType.BF16, init_value=torch.randn),
    ...
    def get_program(self) -> Any:
        ...
        class QKMatmul2BlockProgram:
            ...
            def orchestrator(
                ...
                out = kernel_qk_matmul_2block(qi, kj0, kj1, out)
                return out
    ...
    def compute_expected(self, tensors, params=None):
        qi = tensors["qi"].to(torch.float32)
        kj0 = tensors["kj0"].to(torch.float32)
        kj1 = tensors["kj1"].to(torch.float32)
        sij0 = torch.matmul(qi, kj0.T)
        sij1 = torch.matmul(qi, kj1.T)
        tensors["sij"][:] = torch.cat([sij0, sij1], dim=1)
...
class PVMatmul2BlockTestCase(PTOTestCase):
    ...
    def define_tensors(self) -> list[TensorSpec]:
        return [
            ...
            TensorSpec("vj0", [self.block_size, self.head_dim], DataType.BF16, init_value=torch.randn),
            TensorSpec("vj1", [self.block_size, self.head_dim], DataType.BF16, init_value=torch.randn),
    ...
    def get_program(self) -> Any:
        ...
        class PVMatmul2BlockProgram:
            ...
            def orchestrator(
                ...
                out_oi = kernel_pv_matmul_2block(pij, vj0, vj1, out_oi)
                return out_oi
    ...
    def compute_expected(self, tensors, params=None):
        pij = tensors["pij"].to(torch.float32)
        vj0 = tensors["vj0"].to(torch.float32)
        vj1 = tensors["vj1"].to(torch.float32)
        bs = vj0.shape[0]
        tensors["oi_new"][:] = torch.matmul(pij[:, :bs], vj0) + torch.matmul(pij[:, bs:], vj1)
