...
| `--memory-planner` | `ptoas` | Session-wide on-chip memory planner for cases that do not select one themselves: `ptoas` (ptoas `PlanMemory` allocates), `pypto`, `dsa-rp`, or `default` (whatever `ir.compile()` picks). A planner set on the case itself still wins. |
